...
const scopes = ["User.Read", "Calendars.ReadWrite", "Calendars.ReadWrite.Shared", "Mail.ReadWrite", "Mail.ReadWrite.Shared", "Mail.Send", "Mail.Send.Shared"];
...
export function microsoftAccount() {
  // MSAL v5 throws uninitialized_public_client_application if account APIs
  // are queried before initialize() has completed. React calls this helper
  // during initial render, so return null until restoreMicrosoftSession()
  // has finished initialization.
  if (!msal || !msalInitialized) return null;
...
async function token() {
  ...
  try {
    ...
    try {
      return (await msal.acquireTokenPopup({ account, scopes, redirectUri: popupRedirectUri, overrideInteractionInProgress: true })).accessToken;
    } catch (popupError) {
...
function mailBase(mailbox = "") {
  return mailbox.trim() ? `/users/${encodeURIComponent(mailbox.trim())}` : "/me";
}

export async function listMailMessages({ folder = "inbox", mailbox = "", top = 50 } = {}) {
  const select = "id,subject,from,toRecipients,ccRecipients,receivedDateTime,sentDateTime,isRead,hasAttachments,bodyPreview,body,webLink";
  const qs = new URLSearchParams({ "$top": String(top), "$orderby": folder.toLowerCase()==="sentitems" ? "sentDateTime desc" : "receivedDateTime desc", "$select": select });
  const data = await graph(`${mailBase(mailbox)}/mailFolders/${encodeURIComponent(folder)}/messages?${qs.toString()}`);
  return data.value || [];
}

export async function markMailRead(messageId, { mailbox = "", isRead = true } = {}) {
  return graph(`${mailBase(mailbox)}/messages/${encodeURIComponent(messageId)}`, { method: "PATCH", body: JSON.stringify({ isRead }) });
}

function recipients(raw = "") {
  return String(raw).split(/[,;\n]/).map(x=>x.trim()).filter(x=>x.includes("@")).map(address=>({emailAddress:{address}}));
}

export async function sendMailMessage({ to = "", cc = "", subject = "", body = "", mailbox = "" } = {}) {
  if (!recipients(to).length) throw new Error("Add at least one recipient.");
  const message = { subject: subject || "(No subject)", body: { contentType: "HTML", content: body || "" }, toRecipients: recipients(to), ccRecipients: recipients(cc) };
  return graph(`${mailBase(mailbox)}/sendMail`, { method: "POST", body: JSON.stringify({ message, saveToSentItems: true }) });
}

export async function replyToMailMessage(messageId, { body = "", mailbox = "" } = {}) {
  return graph(`${mailBase(mailbox)}/messages/${encodeURIComponent(messageId)}/reply`, { method: "POST", body: JSON.stringify({ comment: body || "" }) });
}

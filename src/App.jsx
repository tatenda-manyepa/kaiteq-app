...
function OutlookConnectPanel({ meetings, onImport, onUpdate }) {
  ...
  return <Card style={{padding:14,marginBottom:16,background:C.panel2}}><div className="flex items-start justify-between gap-4"><div style={{minWidth:0,flex:1}}><div style={{fontSize:12.5,fontWeight:600,color:C.text}}>Microsoft 365 / Outlook</div><div style={{fontSize:11.5,color:C.mut,marginTop:4}}>{account?`Connected as ${account.username||account.name||"Microsoft 365 user"}`:"Connect your KAI-TEQ Microsoft account to sync meetings."}</div>{account&&<div className="grid grid-cols-2 gap-2 mt-3" style={{maxWidth:720}}><Field label="My Outlook calendar"><select style={inputStyle} value={calendarId} onChange={e=>setCalendarId(e.target.value)} disabled={Boolean(mailbox.trim())}><option value="">Default calendar</option>{calendars.map(c=><option key={c.id} value={c.id}>{c.name}{c.ownerAddress&&c.ownerAddress!==account.username?` — ${c.ownerAddress}`:""}</option>)}</select></Field><Field label="Shared mailbox / colleague calendar (optional)"><input style={inputStyle} value={mailbox} onChange={e=>setMailbox(e.target.value)} placeholder="e.g. info@kai-teq.com or colleague@kai-teq.com"/></Field></div>}{account&&<div style={{fontSize:10.8,color:C.mut2,marginTop:6}}>Leave the shared mailbox field blank to use the selected personal calendar. Enter a shared mailbox or colleague email only where that Microsoft calendar has already been shared/delegated to you.</div>}{msg&&<div style={{fontSize:11,color:/error|timed|failed|graph 4|graph 5/i.test(msg)?C.red:C.cyan,marginTop:6}}>{msg}</div>}</div><div className="flex gap-2 flex-wrap justify-end">{!account?<Btn onClick={connect}><Cloud size={14}/> {busy?"Connecting…":"Connect"}</Btn>:<><Btn onClick={imp}><RefreshCw size={14}/> {busy?"Working…":"Import Outlook"}</Btn><Btn onClick={push}><Upload size={14}/> Send to Outlook</Btn><button onClick={loadCalendars} style={{fontSize:11.5,color:C.cyan}}>Refresh calendars</button><button onClick={disconnect} style={{fontSize:11.5,color:C.mut}}>Disconnect</button></>}</div></div></Card>;
}
...
function EmailWorkspace() {
  ...
  const load=async(f=folder)=>{if(!account)return;setBusy(true);setMsg("");try{const rows=await listMailMessages({folder:f,mailbox});setMessages(rows);setSelected(null)}catch(e){setMsg(e.message||String(e))}finally{setBusy(false)}};
  useEffect(()=>{if(account)load(folder)},[account,folder]);
  ...
  return <div><Topbar title="Email" sub="Read, send and reply to Microsoft 365 email without leaving KAI-TEQ." action={<Btn primary onClick={()=>setCompose(true)}><Plus size={15}/> Compose</Btn>}/>
    <Card style={{padding:14,marginBottom:14,background:C.panel2}}><div className="flex items-center justify-between gap-4"><div><div style={{fontSize:12.5,fontWeight:600}}>Microsoft 365 / Outlook</div><div style={{fontSize:11.5,color:C.mut,marginTop:4}}>{account?`Connected as ${account.username||account.name}`:"Connect Microsoft 365 to use email."}</div></div>{!account?<Btn onClick={connect}><Cloud size={14}/>{busy?"Connecting…":"Connect"}</Btn>:<Btn onClick={()=>load()}><RefreshCw size={14}/>{busy?"Refreshing…":"Refresh"}</Btn>}</div>
    {account&&<div className="mt-3" style={{maxWidth:520}}><Field label="Mailbox"><input style={inputStyle} value={mailbox} onChange={e=>setMailbox(e.target.value)} placeholder="Leave blank for my inbox, or hello@kai-teq.com"/></Field><div style={{fontSize:10.8,color:C.mut2,marginTop:5}}>Leave blank for your personal mailbox. Enter an authorised shared mailbox such as hello@kai-teq.com, then Refresh.</div></div>}{msg&&<div style={{fontSize:11,color:/error|graph 4|graph 5|failed/i.test(msg)?C.red:C.cyan,marginTop:8}}>{msg}</div>}</Card>
    <div className="flex gap-2 mb-3"><button onClick={()=>setFolder("inbox")} className="rounded-lg px-4 py-2" style={{background:folder==="inbox"?"rgba(65,105,255,.16)":C.panel2,border:`1px solid ${folder==="inbox"?C.blue:C.line}`,fontSize:12}}>Inbox</button><button onClick={()=>setFolder("sentitems")} className="rounded-lg px-4 py-2" style={{background:folder==="sentitems"?"rgba(65,105,255,.16)":C.panel2,border:`1px solid ${folder==="sentitems"?C.blue:C.line}`,fontSize:12}}>Sent</button></div>
    <div className="grid gap-3" style={{gridTemplateColumns:selected?"minmax(320px,.8fr) minmax(420px,1.2fr)":"1fr"}}><Card style={{overflow:"hidden"}}>{messages.length===0?<div style={{padding:22,color:C.mut}}>{busy?"Loading messages…":"No messages in this view."}</div>:messages.map(m=><button key={m.id} onClick={()=>open(m)} className="w-full text-left" style={{padding:"13px 15px",borderBottom:`1px solid ${C.line}`,background:selected?.id===m.id?"rgba(65,105,255,.10)":"transparent"}}><div className="flex justify-between gap-3"><div style={{fontSize:12.5,fontWeight:m.isRead?500:700,color:C.text}}>{folder==="sentitems"?`To: ${(m.toRecipients||[]).map(addr).join(", ")||"—"}`:name(m.from)}</div><div style={{fontSize:10.5,color:C.mut2}}>{new Date(m.receivedDateTime||m.sentDateTime).toLocaleString("en-GB",{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"})}</div></div><div style={{fontSize:12,fontWeight:m.isRead?500:700,marginTop:4}}>{m.subject||"(No subject)"}</div><div style={{fontSize:11,color:C.mut,marginTop:3,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{m.bodyPreview}</div></button>)}</Card>

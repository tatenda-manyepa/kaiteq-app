...
import { microsoftConfigured, microsoftSignIn, microsoftSignOut, microsoftAccount, restoreMicrosoftSession, listOutlookCalendars, createOutlookEvent, importOutlookEvents, listMailMessages, markMailRead, sendMailMessage, replyToMailMessage } from "./microsoft";
import {
  ...
  ShieldCheck, Bot, RefreshCw, ExternalLink, Link2, Bell, Settings, LockKeyhole, Cloud, UserCog, FileCheck2
} from "lucide-react";

/* ===========================================================================
   KAI-TEQ — Operating System
   Clients · projects · finance · tasks · meetings · discovery · proposals · AI
   Authentication and the shared KAI-TEQ workspace are connected to Supabase.
=========================================================================== */
...
const EXPENSE_CATEGORIES = ["Software / Hosting", "Domain", "Microsoft 365 / Email", "Contractor", "AI / API", "Travel", "Equipment", "Marketing", "Accounting / Legal", "Insurance", "Banking / Fees", "Other"];
const EXPENSE_TYPES = [
  { key: "overhead", label: "Business overhead", desc: "General KAI-TEQ running cost" },
  { key: "customer", label: "Customer / pre-sales cost", desc: "Cost linked to a customer before or outside a project" },
  { key: "project", label: "Project cost", desc: "Cost attributable to a client project" },
  { key: "product", label: "Internal product cost", desc: "Cost attributable to a KAI-TEQ-owned product" },
];
const REVENUE_TYPES = ["Project delivery", "Consulting", "Implementation", "Support / Retainer", "Internal product", "Other"];
const expenseTypeOf = (e) => e.costType || (e.productId ? "product" : e.projectId ? "project" : e.customerId ? "customer" : "overhead");
...
function loadData() {
  ...
  try {
    ...
    if (raw) {
      ...
      const migratedExpenses = (d.expenses || def.expenses).map((e) => { const x = { category: "Other", projectId: "", productId: "", customerId: "", ...e }; return { ...x, costType: expenseTypeOf(x) }; });
...
function normaliseSharedData(raw, fallback) {
  const base = fallback || loadData();
  const d = raw && typeof raw === "object" ? raw : {};
  const projects = (Array.isArray(d.projects) ? d.projects : base.projects || []).map((p) => ({ ...p, lifecycle: p.lifecycle || (p.stageIdx === STAGES.length - 1 ? "maintenance" : "active") }));
  const expenses = (Array.isArray(d.expenses) ? d.expenses : base.expenses || []).map((e) => { const x = { category: "Other", projectId: "", productId: "", customerId: "", ...e }; return { ...x, costType: expenseTypeOf(x) }; });
  return {
    ...
    meetings: Array.isArray(d.meetings) ? d.meetings : base.meetings || [],
    proposals: Array.isArray(d.proposals) ? d.proposals : base.proposals || [],
    opportunities: Array.isArray(d.opportunities) ? d.opportunities : base.opportunities || [],
    discovery: d.discovery && typeof d.discovery === "object" ? d.discovery : base.discovery || {},
    expenses,
  };
}
...
const NAV = [
  ...
  { key: "pipeline", label: "Business Pipeline", icon: Workflow },
  ...
  { key: "manager", label: "Manager Agent", icon: Bot },
  { key: "products", label: "Internal Products", icon: Package },
  { key: "map", label: "Map", icon: Map },
  { key: "workflow", label: "Workflow Guide", icon: BookOpen },
  { key: "admin", label: "Team & Security", icon: ShieldCheck },
];
function Sidebar({ view, setView, user, role = "member", onLogout }) {
  return (<aside className="flex flex-col justify-between relative" style={{ width: 232, background: "linear-gradient(180deg, rgba(15,20,38,.97), rgba(9,14,28,.96))", borderRight: `1px solid ${C.line}`, padding: 18, flexShrink: 0, boxShadow: "18px 0 50px rgba(0,0,0,.12)", zIndex: 2 }}>
    ...
    <div><div className="flex items-center gap-3 rounded-lg px-3 py-2.5 mb-2" style={{ background: C.panel2 }}><Avatar name={user} /><div style={{ lineHeight: 1.2 }}><div style={{ fontSize: 13, color: C.text, fontWeight: 600 }}>{FULLNAME[user]}</div><div style={{ fontSize: 11, color: C.mut2 }}>{role === "owner" ? "Owner" : role === "admin" ? "Admin" : "Member"}</div></div></div>
...
function Customers({ customers, projects, tasks, meetings, proposals, opportunities = [], discovery, expenses, onNew, onEdit, onDelete, setActiveProject, setView, activeCustomer, setActiveCustomer, onScheduleMeeting }) {
  ...
  if (c) {
    ...
    const cTasks = tasks.filter((t) => t.customerId === c.id || pids.includes(t.projectId));
    ...
    return (<div>
      ...
      {tab==="activity" && (()=>{const timeline=[
        ...cms.map(m=>({id:"m"+m.id,when:m.start||"",title:m.title,kind:"Meeting",detail:`${m.type} · ${m.lead}`})),
        ...qs.map(q=>({id:"q"+q.id,when:q.date||"",title:q.title,kind:"Proposal",detail:`${(PROPOSAL_STATUS.find(x=>x.key===q.status)||{}).label||q.status} · ${gbp(q.value)}`})),
        ...cExpenses.map(e=>({id:"e"+e.id,when:e.date||"",title:e.label,kind:"Cost",detail:`${gbp(e.amount)} · ${e.category||"Other"}`})),
        ...cTasks.map(t=>({id:"t"+t.id,when:t.due||"",title:t.title,kind:"Task",detail:`${t.assignee} · ${(COLS.find(x=>x.key===t.col)||{}).label||t.col}`})),
      ].sort((a,b)=>String(b.when||"").localeCompare(String(a.when||""))); return <div><div className="flex gap-2 mb-3"><button onClick={()=>onScheduleMeeting(c.id)} className="rounded-lg px-3 py-2" style={{background:C.panel2,border:`1px solid ${C.line}`,color:C.cyan,fontSize:11.5}}><Plus size={12} style={{display:"inline"}}/> Schedule meeting</button><button onClick={()=>setView("tasks")} className="rounded-lg px-3 py-2" style={{background:C.panel2,border:`1px solid ${C.line}`,color:C.cyan,fontSize:11.5}}>Open customer tasks</button><button onClick={()=>setView("documents")} className="rounded-lg px-3 py-2" style={{background:C.panel2,border:`1px solid ${C.line}`,color:C.cyan,fontSize:11.5}}>Open documents</button></div><Card style={{padding:18}}><h3 style={{fontSize:13,fontWeight:600,color:C.text,marginBottom:14}}>Customer activity timeline</h3><div className="flex flex-col">{timeline.map((x,i)=><div key={x.id} className="flex gap-3" style={{padding:"0 0 14px"}}><div className="flex flex-col items-center"><div style={{width:9,height:9,borderRadius:9,background:x.kind==="Meeting"?C.cyan:x.kind==="Proposal"?C.purple:x.kind==="Cost"?C.amber:C.blue,marginTop:5}}/>{i<timeline.length-1&&<div style={{width:1,flex:1,background:C.line,marginTop:4}}/>}</div><div className="flex-1"><div className="flex items-center justify-between gap-3"><span style={{fontSize:12.5,color:C.text,fontWeight:600}}>{x.title}</span><span style={{fontSize:10.5,color:C.mut2}}>{x.when?new Date(x.when).toLocaleDateString("en-GB"):"No date"}</span></div><div style={{fontSize:11,color:C.mut,marginTop:3}}>{x.kind} · {x.detail}</div></div></div>)}{!timeline.length&&<span style={{fontSize:12,color:C.mut2}}>No customer activity yet.</span>}</div></Card></div>})()}
      ...
      {tab==="docs" && <Card style={{padding:18}}><div className="flex items-center justify-between"><div><h3 style={{fontSize:13,fontWeight:600,color:C.text}}>Shared customer documents</h3><p style={{fontSize:12,color:C.mut,marginTop:4}}>V9 stores receipts, contracts, proposals and evidence securely in Supabase Storage. Open Documents to upload or view files linked to this customer.</p></div><Btn onClick={()=>setView("documents")}><Paperclip size={14}/> Open Documents</Btn></div></Card>}
    </div>);
  }
...
function ProjectModal({ customers, project, onClose, onSave }) {
  const [f, setF] = useState(project || { id: "p" + Date.now(), customerId: customers[0]?.id || "", name: "", lead: "Tatenda", health: "on", stageIdx: 0, lifecycle: "planned", tags: [], value: 0, paid: 0, retainer: 0, revenueRecognised: 0, invoiceStatus: "none", revenueType: "Project delivery" });
  ...
  return (<Modal wide title={project?"Edit project":"New project"} onClose={onClose} onSave={save} saveLabel={project?"Save project":"Create project"}>
    ...
    <div style={{fontSize:12,color:C.cyan,fontWeight:600,marginTop:4}}>Commercials</div><div className="grid grid-cols-3 gap-3"><Field label="Project value (£)"><input style={inputStyle} type="number" value={f.value} onChange={e=>set("value",e.target.value)}/></Field><Field label="Paid so far (£)"><input style={inputStyle} type="number" value={f.paid} onChange={e=>set("paid",e.target.value)}/></Field><Field label="Revenue recognised (£)"><input style={inputStyle} type="number" value={f.revenueRecognised} onChange={e=>set("revenueRecognised",e.target.value)}/></Field><Field label="Monthly support (£)"><input style={inputStyle} type="number" value={f.retainer} onChange={e=>set("retainer",e.target.value)}/></Field><Field label="Revenue type"><select style={inputStyle} value={f.revenueType || "Project delivery"} onChange={e=>set("revenueType",e.target.value)}>{REVENUE_TYPES.map(x=><option key={x}>{x}</option>)}</select></Field><Field label="Invoice status"><select style={inputStyle} value={f.invoiceStatus} onChange={e=>set("invoiceStatus",e.target.value)}>{INVOICE_STATUS.map(x=><option key={x.key} value={x.key}>{x.label}</option>)}</select></Field></div>
  </Modal>);
}
...
/* --------------------------- Finance --------------------------- */
function ExpenseModal({ customers = [], projects, products = [], presetProjectId="", presetProductId="", presetCustomerId="", onClose, onCreate }) {
  const presetProject=projects.find(p=>p.id===presetProjectId);
  const initialType=presetProjectId?"project":presetProductId?"product":presetCustomerId?"customer":"overhead";
  const [f,setF]=useState({label:"",amount:"",date:new Date().toISOString().slice(0,10),category:"Other",costType:initialType,projectId:presetProjectId,productId:presetProductId,customerId:presetCustomerId||presetProject?.customerId||"",vendor:"",vat:"",paymentMethod:"Card",reference:"",notes:""});
  const set=(k,v)=>setF(x=>({...x,[k]:v}));
  const save=()=>{if(!f.label.trim())return;const p=projects.find(x=>x.id===f.projectId);onCreate({id:"e"+Date.now(),label:f.label.trim(),amount:Number(f.amount)||0,date:f.date,category:f.category,costType:f.costType,projectId:f.costType==="project"?(f.projectId||""):"",productId:f.costType==="product"?(f.productId||""):"",customerId:f.costType==="project"?(p?.customerId||f.customerId||""):f.costType==="customer"?(f.customerId||""):"",vendor:f.vendor.trim(),vat:Number(f.vat)||0,paymentMethod:f.paymentMethod,reference:f.reference.trim(),notes:f.notes.trim()});onClose();};
  return <Modal wide title="Add cost / expense" onClose={onClose} onSave={save} saveLabel="Add cost">
    <Field label="Description"><input style={inputStyle} value={f.label} onChange={e=>set("label",e.target.value)} placeholder="e.g. Microsoft 365 licences, domain renewal, discovery travel"/></Field>
    <div className="grid grid-cols-3 gap-3">
      <Field label="Amount (£)"><input style={inputStyle} type="number" value={f.amount} onChange={e=>set("amount",e.target.value)}/></Field>
      <Field label="VAT (£)"><input style={inputStyle} type="number" value={f.vat} onChange={e=>set("vat",e.target.value)}/></Field>
      <Field label="Date"><input style={inputStyle} type="date" value={f.date} onChange={e=>set("date",e.target.value)}/></Field>
      <Field label="Cost type"><select style={inputStyle} value={f.costType} onChange={e=>{const t=e.target.value;setF(x=>({...x,costType:t,projectId:t==="project"?x.projectId:"",productId:t==="product"?x.productId:"",customerId:["project","customer"].includes(t)?x.customerId:""}))}}>{EXPENSE_TYPES.map(x=><option key={x.key} value={x.key}>{x.label}</option>)}</select></Field>
      <Field label="Category"><select style={inputStyle} value={f.category} onChange={e=>set("category",e.target.value)}>{EXPENSE_CATEGORIES.map(x=><option key={x}>{x}</option>)}</select></Field>
      <Field label="Vendor / supplier"><input style={inputStyle} value={f.vendor} onChange={e=>set("vendor",e.target.value)} placeholder="Microsoft, GoDaddy…"/></Field>
      {f.costType==="customer"&&<Field label="Customer"><select style={inputStyle} value={f.customerId} onChange={e=>set("customerId",e.target.value)}><option value="">Select customer</option>{customers.map(c=><option key={c.id} value={c.id}>{c.company}</option>)}</select></Field>}
      {f.costType==="project"&&<Field label="Project"><select style={inputStyle} value={f.projectId} onChange={e=>{const pid=e.target.value;const p=projects.find(x=>x.id===pid);setF(x=>({...x,projectId:pid,customerId:p?.customerId||""}))}}><option value="">Select project</option>{projects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></Field>}
      {f.costType==="product"&&<Field label="Internal product"><select style={inputStyle} value={f.productId} onChange={e=>set("productId",e.target.value)}><option value="">Select product</option>{products.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></Field>}
      <Field label="Payment method"><select style={inputStyle} value={f.paymentMethod} onChange={e=>set("paymentMethod",e.target.value)}>{["Card","Bank transfer","Direct debit","Cash","Other"].map(x=><option key={x}>{x}</option>)}</select></Field>
      <Field label="Reference / invoice no."><input style={inputStyle} value={f.reference} onChange={e=>set("reference",e.target.value)}/></Field>
    </div>
    <Field label="Notes"><textarea rows={2} style={{...inputStyle,resize:"vertical"}} value={f.notes} onChange={e=>set("notes",e.target.value)} /></Field>
    <div className="rounded-lg p-3" style={{background:C.panel2,border:`1px solid ${C.line}`,fontSize:11.5,color:C.mut,lineHeight:1.5}}>{(EXPENSE_TYPES.find(x=>x.key===f.costType)||EXPENSE_TYPES[0]).desc}. Receipts and invoices can be uploaded in Documents and linked to this cost using its reference.</div>
  </Modal>;
}
function Finance({ customers, projects, products = [], proposals, expenses, onInvoiceStatus, onAddExpense, onDeleteExpense }) {
  const revenue=projects.reduce((a,p)=>a+(Number(p.revenueRecognised)||0),0);
  const projectExp=expenses.filter(e=>expenseTypeOf(e)==="project").reduce((a,e)=>a+(Number(e.amount)||0),0);
  const customerExp=expenses.filter(e=>expenseTypeOf(e)==="customer").reduce((a,e)=>a+(Number(e.amount)||0),0);
  const overheadExp=expenses.filter(e=>expenseTypeOf(e)==="overhead").reduce((a,e)=>a+(Number(e.amount)||0),0);
  const productExp=expenses.filter(e=>expenseTypeOf(e)==="product").reduce((a,e)=>a+(Number(e.amount)||0),0);
  const exp=projectExp+customerExp+overheadExp+productExp;
  const outstanding=projects.reduce((a,p)=>a+Math.max(0,(p.value||0)-(p.paid||0)),0);
  const recurring=projects.filter(p=>p.lifecycle==="maintenance").reduce((a,p)=>a+(p.retainer||0),0);
  const profit=revenue-exp, margin=revenue?profit/revenue*100:0;
  const [showExp,setShowExp]=useState(false);
  const stats=[
    {label:"Revenue",value:gbp(revenue),accent:C.green},
    {label:"Project costs",value:gbp(projectExp),accent:C.amber},
    {label:"Pre-sales costs",value:gbp(customerExp),accent:C.blue},
    {label:"Business overheads",value:gbp(overheadExp),accent:C.orange},
    {label:"Product costs",value:gbp(productExp),accent:C.purple},
    {label:"Total costs",value:gbp(exp),accent:C.amber},
    {label:"Operating profit",value:gbp(profit),accent:profit>=0?C.cyan:C.red},
    {label:"Margin",value:`${margin.toFixed(1)}%`,accent:C.purple},
    {label:"Outstanding",value:gbp(outstanding),accent:C.orange},
  ];
  ...
  const costList=(type,title,subtitle)=>{const rows=expenses.filter(e=>expenseTypeOf(e)===type);const total=rows.reduce((a,e)=>a+(Number(e.amount)||0),0);return <div><div className="flex items-end justify-between mb-2"><div><h3 style={{fontSize:13,fontWeight:600,color:C.text}}>{title}</h3><div style={{fontSize:10.5,color:C.mut2,marginTop:2}}>{subtitle}</div></div><span style={{fontSize:13,fontWeight:600,color:C.amber}}>{gbp(total)}</span></div><Card style={{padding:0,overflow:"hidden"}}>{rows.map(e=>{const link=type==="project"?projName(projects,e.projectId):type==="customer"?custName(customers,e.customerId):type==="product"?(products.find(p=>p.id===e.productId)?.name||"Internal product"):"KAI-TEQ";return <div key={e.id} className="flex items-center justify-between" style={{padding:"11px 14px",borderBottom:`1px solid ${C.line}`}}><div><div style={{fontSize:12.5,color:C.text}}>{e.label}</div><div style={{fontSize:10.5,color:C.mut2}}>{e.category||"Other"} · {link} · {e.date}</div></div><div className="flex items-center gap-3"><span style={{fontSize:12.5,color:C.amber}}>{gbp(e.amount)}</span><button onClick={()=>onDeleteExpense(e.id)} style={{color:C.mut2}}><X size={14}/></button></div></div>})}{!rows.length&&<div style={{padding:14,fontSize:12,color:C.mut2}}>No costs recorded.</div>}</Card></div>};
  return <div><Topbar title="Finance" sub="KAI-TEQ company performance — revenue, project costs, overheads, internal products and operating profit." action={<Btn onClick={()=>setShowExp(true)}><Plus size={15}/> Add cost</Btn>}/>
    <div className="grid grid-cols-4 gap-3 mb-4">{stats.map(x=><Stat key={x.label} {...x}/>)}</div>
    <div className="flex items-center gap-2 mb-6" style={{fontSize:11.5,color:C.mut2}}><span>Recurring support:</span><span style={{color:C.blue,fontWeight:600}}>{gbp(recurring)}/mo</span><span>·</span><span>Operating profit = recognised project revenue − all KAI-TEQ costs.</span></div>
    <h2 style={{fontSize:14,fontWeight:600,color:C.text,marginBottom:12}}>Company costs</h2>
    <div className="grid grid-cols-2 gap-4 mb-7">{costList("overhead","Business overheads","Company-wide running costs")}{costList("customer","Customer / pre-sales costs","Discovery, sales and account costs")}{costList("project","Project costs","Direct client delivery costs")}{costList("product","Internal product costs","KAI-TEQ-owned product investment")}</div>
    <h2 style={{fontSize:14,fontWeight:600,color:C.text,marginBottom:12}}>Project profitability</h2><Card style={{padding:0,marginBottom:24,overflow:"hidden"}}><div className="grid" style={{gridTemplateColumns:"1.7fr 1.2fr .8fr .8fr .8fr .7fr .9fr",padding:"12px 16px",fontSize:11,color:C.mut2,borderBottom:`1px solid ${C.line}`}}><span>PROJECT</span><span>CUSTOMER</span><span>REVENUE</span><span>COSTS</span><span>PROFIT</span><span>MARGIN</span><span>STATUS</span></div>{projects.map(p=>{const st=projectStatus(p),pr=projectProfit(p,expenses);return <div key={p.id} className="grid items-center" style={{gridTemplateColumns:"1.7fr 1.2fr .8fr .8fr .8fr .7fr .9fr",padding:"12px 16px",fontSize:12.5,color:C.text,borderBottom:`1px solid ${C.line}`}}><span>{p.name}</span><span style={{color:C.mut}}>{custName(customers,p.customerId)}</span><span>{gbp(p.revenueRecognised)}</span><span style={{color:C.amber}}>{gbp(projectCosts(expenses,p.id))}</span><span style={{color:pr>=0?C.green:C.red}}>{gbp(pr)}</span><span>{projectMargin(p,expenses).toFixed(1)}%</span><Chip label={st.label} color={st.color}/></div>})}{!projects.length&&<div style={{padding:14,fontSize:12,color:C.mut2}}>No projects yet.</div>}</Card>
    <h2 style={{fontSize:14,fontWeight:600,color:C.text,marginBottom:12}}>Invoices & receivables</h2><Card style={{padding:0,marginBottom:24,overflow:"hidden"}}>{projects.map(p=>{const out=Math.max(0,(p.value||0)-(p.paid||0));const st=INVOICE_STATUS.find(s=>s.key===p.invoiceStatus)||INVOICE_STATUS[0];return <div key={p.id} className="grid items-center" style={{gridTemplateColumns:"2fr 1.3fr 1fr 1fr 1.2fr",padding:"12px 16px",fontSize:12.5,color:C.text,borderBottom:`1px solid ${C.line}`}}><span>{p.name}</span><span style={{color:C.mut}}>{custName(customers,p.customerId)}</span><span>{gbp(p.paid)} paid</span><span style={{color:out?C.orange:C.mut2}}>{gbp(out)} due</span><select value={p.invoiceStatus} onChange={e=>onInvoiceStatus(p.id,e.target.value)} style={{background:C.panel2,color:st.color,border:`1px solid ${st.color}44`,borderRadius:6,fontSize:12,padding:"4px 6px",outline:"none"}}>{INVOICE_STATUS.map(x=><option key={x.key} value={x.key} style={{background:C.panel2,color:C.text}}>{x.label}</option>)}</select></div>})}{!projects.length&&<div style={{padding:14,fontSize:12,color:C.mut2}}>No invoices yet.</div>}</Card>
    <h2 style={{fontSize:14,fontWeight:600,color:C.text,marginBottom:12}}>By customer</h2><Card style={{padding:0,overflow:"hidden"}}>{customers.map(c=>{const ps=projects.filter(p=>p.customerId===c.id);const rev=ps.reduce((a,p)=>a+(p.revenueRecognised||0),0);const cost=expenses.filter(e=>expenseTypeOf(e)==="project"&&(e.customerId===c.id||ps.some(p=>p.id===e.projectId))).reduce((a,e)=>a+(Number(e.amount)||0),0);return <div key={c.id} className="flex items-center justify-between" style={{padding:"12px 16px",borderBottom:`1px solid ${C.line}`}}><div><div style={{fontSize:12.5,color:C.text,fontWeight:600}}>{c.company}</div><div style={{fontSize:10.5,color:C.mut2}}>{gbp(rev)} revenue · {gbp(cost)} direct costs · {gbp(rev-cost)} project contribution</div></div><button onClick={()=>exportCustomer(c)} style={{fontSize:11.5,color:C.cyan}}><Download size={13} style={{display:"inline"}}/> Export</button></div>})}{!customers.length&&<div style={{padding:14,fontSize:12,color:C.mut2}}>No customers yet.</div>}</Card>
    {showExp&&<ExpenseModal customers={customers} projects={projects} products={products} onClose={()=>setShowExp(false)} onCreate={onAddExpense}/>}</div>;
}

/* --------------------------- Tasks --------------------------- */
function TaskModal({ customers = [], projects, products = [], preset, presetCustomerId = "", onClose, onCreate }) {
  const [f, setF] = useState({ title: "", projectId: preset || (projects[0]?.id || "internal"), customerId: presetCustomerId, type: "feature", col: "todo", assignee: "Tatenda", due: "" });
  const set = (k, v) => setF((s) => ({ ...s, [k]: v }));
  const target = f.customerId ? `customer:${f.customerId}` : f.projectId;
  const setTarget=(v)=>{if(v.startsWith("customer:")) setF(x=>({...x,customerId:v.slice(9),projectId:""})); else setF(x=>({...x,projectId:v,customerId:""}));};
  ...
  return (<Modal title="Add task" onClose={onClose} onSave={save} saveLabel="Add task">
    <Field label="Task"><input style={inputStyle} value={f.title} onChange={(e) => set("title", e.target.value)} placeholder="e.g. Follow up after discovery" /></Field>
    <Field label="Link task to"><select style={inputStyle} value={target} onChange={(e) => setTarget(e.target.value)}>{customers.map(c=><option key={c.id} value={`customer:${c.id}`}>{c.company} (customer / pre-sales)</option>)}{projects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}{products.map((p) => <option key={p.id} value={p.id}>{p.name} (product)</option>)}<option value="internal">Internal / KAI-TEQ</option></select></Field>
    <div className="grid grid-cols-2 gap-3">
...
function Tasks({ customers = [], projects, products = [], tasks, onMoveTask, onDeleteTask, onNew }) {
  const [filter, setFilter] = useState("all");
  const [who, setWho] = useState("all");
  const nameFor = (id, customerId) => customerId ? `${custName(customers, customerId)} · customer` : id === "internal" ? "Internal / KAI-TEQ" : ((projects.find((p) => p.id === id) || {}).name || (products.find((p) => p.id === id) || {}).name || "—");
  let shown = filter === "all" ? tasks : filter.startsWith("customer:") ? tasks.filter(t=>t.customerId===filter.slice(9)) : tasks.filter((t) => t.projectId === filter);
  ...
  return (<div><Topbar title="Tasks" sub="Sprint work, bugs, client and meeting actions — across everything." action={<Btn primary onClick={() => onNew(null)}><Plus size={15} /> Add task</Btn>} />
    <div className="flex items-center gap-2 mb-4">
      <select value={filter} onChange={(e) => setFilter(e.target.value)} style={{ ...inputStyle, width: "auto", padding: "7px 11px" }}><option value="all">All work</option>{customers.map(c=><option key={`c-${c.id}`} value={`customer:${c.id}`}>{c.company} (customer)</option>)}{projects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}{products.map((p) => <option key={p.id} value={p.id}>{p.name} (product)</option>)}<option value="internal">Internal / KAI-TEQ</option></select>
      ...
      return (<div key={col.key} className="rounded-xl p-2.5" style={{ background: C.panel, border: `1px solid ${C.line}` }}>
        ...
        <div className="flex flex-col gap-2">{items.map((t) => { const type = TASK_TYPES.find((x) => x.key === t.type) || TASK_TYPES[0]; const di = dueInfo(t.due);
          return (<div key={t.id} className="rounded-lg p-2.5" style={{ background: C.panel2, border: `1px solid ${C.line}` }}>
            ...
            <div style={{ fontSize: 10.5, color: C.mut2, marginBottom: 6 }}>{nameFor(t.projectId, t.customerId)}</div>
...
function Meetings({ customers, projects = [], meetings, onUpdate, onDelete, onNew, onCreateTask, onImport }) {
  ...
  return (<div><Topbar title="Calendar" sub="KAI-TEQ meetings and Microsoft 365 / Outlook in one connected calendar." action={<Btn primary onClick={onNew}><Plus size={15} /> Schedule meeting</Btn>} />
    <OutlookConnectPanel meetings={meetings} onImport={onImport} onUpdate={onUpdate}/>
...
/* --------------------------- V9 Business Pipeline --------------------------- */
function BusinessPipeline({ customers, projects, proposals, opportunities, tasks, setView }) {
  const stages=[
    {k:"customers",label:"Customer",count:customers.filter(c=>c.status==="Lead").length,desc:"Lead / account created",color:C.mut},
    {k:"discovery",label:"Discovery",count:customers.filter(c=>c.status==="Discovery").length,desc:"Problem, value and success criteria",color:C.cyan},
    {k:"ai",label:"Opportunity",count:opportunities.filter(o=>!["declined","delivered"].includes(o.status)).length,desc:"Qualified AI / automation opportunity",color:C.purple},
    {k:"proposals",label:"Proposal",count:proposals.filter(q=>["draft","sent"].includes(q.status)).length,desc:"Scope, value and decision",color:C.blue},
    {k:"projects",label:"Project",count:projects.filter(p=>!["maintenance","archived"].includes(p.lifecycle)).length,desc:"Won work becomes delivery",color:C.green},
    {k:"tasks",label:"Delivery",count:tasks.filter(t=>t.projectId&&t.projectId!=="internal"&&t.col!=="done").length,desc:"Tasks, milestones and meetings",color:C.amber},
    {k:"projects",label:"Support",count:projects.filter(p=>p.lifecycle==="maintenance").length,desc:"Maintenance, retainer and growth",color:C.orange},
  ];
  return <div><Topbar title="Connected Business Pipeline" sub="Customer → Discovery → Opportunity → Proposal → Project → Delivery → Support. Each stage is linked to the same customer record."/>
    <div className="grid grid-cols-7 gap-2 mb-6">{stages.map((x,i)=><Card key={i} className="cursor-pointer" style={{padding:15,minHeight:170}}><div onClick={()=>setView(x.k)}><div style={{fontSize:10,color:x.color,fontWeight:700,letterSpacing:".08em"}}>0{i+1}</div><div style={{fontSize:15,color:C.text,fontWeight:650,marginTop:12}}>{x.label}</div><div style={{fontSize:30,color:C.text,fontWeight:700,marginTop:8}}>{x.count}</div><div style={{fontSize:11,color:C.mut,lineHeight:1.45,marginTop:8}}>{x.desc}</div><div style={{height:3,borderRadius:3,background:x.color,marginTop:14}}/></div></Card>)}</div>
    <Card style={{padding:18}}><div className="flex items-start justify-between"><div><div style={{fontSize:13.5,fontWeight:650,color:C.text}}>V9 lifecycle automation</div><div style={{fontSize:12,color:C.mut,marginTop:6,lineHeight:1.65,maxWidth:850}}>Discovery population moves the account toward Proposal. Sending/accepting proposals updates the customer lifecycle; accepted work can create a delivery project. Maintenance projects move the account into Retainer. Customer-level pre-sales tasks and costs stay visible before a project exists.</div></div><Chip label="Connected" color={C.green}/></div></Card>
  </div>;
}

/* --------------------------- V9 Shared Documents --------------------------- */
function DocumentsV9({ customers, projects, products, expenses, role }) {
  const [docs,setDocs]=useState([]); const [busy,setBusy]=useState(false); const [err,setErr]=useState("");
  const [f,setF]=useState({kind:"Other",customerId:"",projectId:"",productId:"",expenseId:"",notes:""});
  const load=async()=>{if(!supabase)return;const {data,error}=await supabase.from("documents").select("*").order("created_at",{ascending:false});if(error)setErr(error.message);else setDocs(data||[])};
  useEffect(()=>{load()},[]);
  const upload=async(e)=>{const file=e.target.files?.[0]; if(!file||!supabase)return; setBusy(true);setErr(""); try{const safe=file.name.replace(/[^a-zA-Z0-9._-]+/g,"-");const path=`${new Date().toISOString().slice(0,10)}/${Date.now()}-${safe}`;const {error:upErr}=await supabase.storage.from("kai-teq-documents").upload(path,file,{contentType:file.type||"application/octet-stream"});if(upErr)throw upErr;const {error:dbErr}=await supabase.from("documents").insert({name:file.name,storage_path:path,mime_type:file.type,size_bytes:file.size,document_type:f.kind,customer_id:f.customerId||null,project_id:f.projectId||null,product_id:f.productId||null,expense_id:f.expenseId||null,notes:f.notes||null});if(dbErr)throw dbErr;setF({kind:"Other",customerId:"",projectId:"",productId:"",expenseId:"",notes:""});await load();}catch(x){setErr(x.message||String(x))}finally{setBusy(false);e.target.value=""}};
  const openDoc=async(d)=>{const {data,error}=await supabase.storage.from("kai-teq-documents").createSignedUrl(d.storage_path,300);if(error)setErr(error.message);else window.open(data.signedUrl,"_blank","noopener,noreferrer")};
  const del=async(d)=>{if(!["owner","admin"].includes(role))return window.alert("Only an Owner or Admin can delete shared documents.");if(!window.confirm(`Delete ${d.name}?`))return;await supabase.storage.from("kai-teq-documents").remove([d.storage_path]);await supabase.from("documents").delete().eq("id",d.id);load()};
  const customerName=id=>custName(customers,id); const projectName=id=>projName(projects,id);
  return <div><Topbar title="Shared Documents & Receipts" sub="Secure Supabase Storage for proposals, contracts, receipts, invoices and delivery evidence."/>
    <Card style={{padding:16,marginBottom:18}}><div className="grid grid-cols-6 gap-3 items-end"><Field label="Type"><select style={inputStyle} value={f.kind} onChange={e=>setF(x=>({...x,kind:e.target.value}))}>{["Receipt","Invoice","Proposal","Contract","Discovery","Delivery evidence","Other"].map(x=><option key={x}>{x}</option>)}</select></Field><Field label="Customer"><select style={inputStyle} value={f.customerId} onChange={e=>setF(x=>({...x,customerId:e.target.value}))}><option value="">—</option>{customers.map(c=><option key={c.id} value={c.id}>{c.company}</option>)}</select></Field><Field label="Project"><select style={inputStyle} value={f.projectId} onChange={e=>setF(x=>({...x,projectId:e.target.value}))}><option value="">—</option>{projects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></Field><Field label="Product"><select style={inputStyle} value={f.productId} onChange={e=>setF(x=>({...x,productId:e.target.value}))}><option value="">—</option>{products.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></Field><Field label="Cost / receipt link"><select style={inputStyle} value={f.expenseId} onChange={e=>setF(x=>({...x,expenseId:e.target.value}))}><option value="">—</option>{expenses.slice(0,100).map(x=><option key={x.id} value={x.id}>{x.label}</option>)}</select></Field><label className="rounded-lg px-4 py-2 text-center cursor-pointer" style={{background:GRAD,color:"#fff",fontSize:12.5,fontWeight:650}}>{busy?"Uploading…":"Upload file"}<input type="file" disabled={busy} onChange={upload} style={{display:"none"}}/></label></div>{err&&<div style={{color:C.red,fontSize:11.5,marginTop:10}}>{err}</div>}</Card>
    <div className="flex flex-col gap-2">{docs.map(d=><Card key={d.id} style={{padding:14}}><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><FileCheck2 size={18} style={{color:C.cyan}}/><div><div style={{fontSize:13,color:C.text,fontWeight:600}}>{d.name}</div><div style={{fontSize:10.8,color:C.mut2,marginTop:3}}>{d.document_type||"Other"}{d.customer_id?` · ${customerName(d.customer_id)}`:""}{d.project_id?` · ${projectName(d.project_id)}`:""} · {new Date(d.created_at).toLocaleDateString("en-GB")}</div></div></div><div className="flex gap-2"><button onClick={()=>openDoc(d)} className="rounded-lg px-3 py-2" style={{background:C.panel2,color:C.cyan,border:`1px solid ${C.line}`,fontSize:11.5}}><ExternalLink size={13} style={{display:"inline",marginRight:5}}/>Open</button>{["owner","admin"].includes(role)&&<button onClick={()=>del(d)} style={{color:C.red}}><Trash2 size={15}/></button>}</div></div></Card>)}{!docs.length&&!busy&&<Card style={{padding:18}}><span style={{fontSize:12.5,color:C.mut2}}>No shared documents uploaded yet.</span></Card>}</div></div>;
}

function MicrosoftMark() {
  const squares=["#f35325","#81bc06","#05a6f0","#ffba08"];
  return <span aria-hidden="true" style={{display:"grid",gridTemplateColumns:"repeat(2,8px)",gap:2,width:18,height:18,flex:"0 0 auto"}}>{squares.map((color,i)=><span key={i} style={{background:color,borderRadius:1}}/>)}</span>;
}

/* --------------------------- V9 Microsoft 365 --------------------------- */
function OutlookConnectPanel({ meetings, onImport, onUpdate }) {
  const [account,setAccount]=useState(microsoftAccount());
  const [busy,setBusy]=useState(false);
  const [msg,setMsg]=useState("");
  const [calendars,setCalendars]=useState([]);
  const [calendarId,setCalendarId]=useState("");
  const [mailbox,setMailbox]=useState(()=>localStorage.getItem("kai_teq_ms_shared_mailbox")||"");
  const loadCalendars=async()=>{try{const rows=await listOutlookCalendars();setCalendars(rows);if(!calendarId){const d=rows.find(c=>c.isDefaultCalendar)||rows[0];if(d)setCalendarId(d.id)}}catch(e){setMsg(e.message||String(e))}};
  useEffect(()=>{if(!microsoftConfigured)return;restoreMicrosoftSession().then(a=>{if(a){setAccount(a);loadCalendars()}}).catch(()=>{})},[]);
  const connect=async()=>{setBusy(true);setMsg("");try{const a=await microsoftSignIn();setAccount(a);await loadCalendars();setMsg("Microsoft 365 connected.")}catch(e){setMsg(e.message||String(e))}finally{setBusy(false)}};
  const disconnect=async()=>{setBusy(true);try{await microsoftSignOut();setAccount(null);setCalendars([]);setMsg("Disconnected from Microsoft 365.")}catch(e){setMsg(e.message||String(e))}finally{setBusy(false)}};
  const source=()=>mailbox.trim()?{mailbox:mailbox.trim()}:{calendarId};
  const imp=async()=>{setBusy(true);setMsg("");try{localStorage.setItem("kai_teq_ms_shared_mailbox",mailbox.trim());const rows=await importOutlookEvents(source());let n=0;for(const x of rows){if(!meetings.some(m=>m.outlookEventId===x.outlookEventId)){onImport(x);n++}}setMsg(`${n} Outlook event${n===1?"":"s"} imported.`)}catch(e){setMsg(e.message||String(e))}finally{setBusy(false)}};
  const push=async()=>{const pending=meetings.filter(m=>m.start&&!m.outlookEventId);if(!pending.length)return setMsg("No unsynced KAI-TEQ meetings to send.");setBusy(true);setMsg("");let n=0;try{localStorage.setItem("kai_teq_ms_shared_mailbox",mailbox.trim());for(const m of pending){const ev=await createOutlookEvent(m,source());onUpdate(m.id,{outlookEventId:ev.id,outlookWebLink:ev.webLink||"",outlookSource:mailbox.trim()||calendarId||"me"});n++}setMsg(`${n} meeting${n===1?"":"s"} sent to Outlook.`)}catch(e){setMsg(e.message||String(e))}finally{setBusy(false)}};
  if(!microsoftConfigured) return <Card style={{padding:14,marginBottom:16,background:C.panel2}}><div className="flex items-center justify-between"><div><div style={{fontSize:12.5,fontWeight:600,color:C.text}}>Microsoft 365 integration</div><div style={{fontSize:11.5,color:C.mut,marginTop:4}}>Add VITE_MS_CLIENT_ID and VITE_MS_TENANT_ID in Netlify and register both KAI-TEQ SPA redirect URIs in Microsoft Entra.</div></div><Chip label="Setup required" color={C.amber}/></div></Card>;
  return <Card style={{padding:14,marginBottom:16,background:C.panel2}}><div className="flex items-start justify-between gap-4"><div style={{minWidth:0,flex:1}}><div className="flex items-center gap-2"><MicrosoftMark/><div style={{fontSize:12.5,fontWeight:600,color:C.text}}>Microsoft 365 / Outlook</div>{account&&<span style={{fontSize:10.5,fontWeight:700,color:C.green,border:`1px solid ${C.green}`,borderRadius:999,padding:"2px 8px"}}>Connected</span>}</div><div style={{fontSize:11.5,color:C.mut,marginTop:4,marginLeft:26}}>{account?`Connected as ${account.username||account.name||"Microsoft 365 user"}`:"Connect your KAI-TEQ Microsoft account to sync meetings."}</div>{account&&<div className="grid grid-cols-2 gap-2 mt-3" style={{maxWidth:720}}><Field label="My Outlook calendar"><select style={inputStyle} value={calendarId} onChange={e=>setCalendarId(e.target.value)} disabled={Boolean(mailbox.trim())}><option value="">Default calendar</option>{calendars.map(c=><option key={c.id} value={c.id}>{c.name}{c.ownerAddress&&c.ownerAddress!==account.username?` — ${c.ownerAddress}`:""}</option>)}</select></Field><Field label="Shared mailbox / colleague calendar (optional)"><input style={inputStyle} value={mailbox} onChange={e=>setMailbox(e.target.value)} placeholder="e.g. info@kai-teq.com or colleague@kai-teq.com"/></Field></div>}{account&&<div style={{fontSize:10.8,color:C.mut2,marginTop:6}}>Leave the shared mailbox field blank to use the selected personal calendar. Enter a shared mailbox or colleague email only where that Microsoft calendar has already been shared/delegated to you.</div>}{msg&&<div style={{fontSize:11,color:/error|timed|failed|graph 4|graph 5/i.test(msg)?C.red:C.cyan,marginTop:6}}>{msg}</div>}</div><div className="flex gap-2 flex-wrap justify-end">{!account?<Btn onClick={connect}><Cloud size={14}/> {busy?"Connecting…":"Connect"}</Btn>:<><Btn onClick={imp}><RefreshCw size={14}/> {busy?"Working…":"Import Outlook"}</Btn><Btn onClick={push}><Upload size={14}/> Send to Outlook</Btn><button onClick={loadCalendars} style={{fontSize:11.5,color:C.cyan}}>Refresh calendars</button><button onClick={disconnect} style={{fontSize:11.5,color:C.mut}}>Disconnect</button></>}</div></div></Card>;
}

/* --------------------------- V9 Manager Agent --------------------------- */
function ManagerAgent({ customers, projects, tasks, meetings, proposals, opportunities, expenses, setView }) {
  const today=new Date();today.setHours(0,0,0,0); const overdue=tasks.filter(t=>t.col!=="done"&&t.due&&new Date(t.due)<today); const atRisk=customers.filter(c=>c.health==="risk"); const outstanding=projects.reduce((a,p)=>a+Math.max(0,(Number(p.value)||0)-(Number(p.paid)||0)),0); const sent=proposals.filter(p=>p.status==="sent"); const highOpp=opportunities.filter(o=>oppPriority(o).label==="High priority"&&!["declined","delivered"].includes(o.status)); const profit=projects.reduce((a,p)=>a+(Number(p.revenueRecognised)||0),0)-expenses.reduce((a,e)=>a+(Number(e.amount)||0),0); const soon=meetings.filter(m=>m.start&&new Date(m.start).getTime()>=Date.now()&&new Date(m.start).getTime()<=Date.now()+7*86400000);
  const alerts=[
    {n:overdue.length,title:"Overdue tasks",body:overdue.length?`${overdue.length} task${overdue.length===1?" is":"s are"} overdue and should be reviewed.`:"No overdue work detected.",view:"tasks",color:overdue.length?C.red:C.green},
    {n:sent.length,title:"Proposal follow-ups",body:sent.length?`${sent.length} sent proposal${sent.length===1?" is":"s are"} awaiting a decision.`:"No proposals currently waiting for a decision.",view:"proposals",color:sent.length?C.amber:C.green},
    {n:atRisk.length,title:"Customer health",body:atRisk.length?`${atRisk.length} customer${atRisk.length===1?" needs":"s need"} attention.`:"No customers marked at risk.",view:"customers",color:atRisk.length?C.red:C.green},
    {n:highOpp.length,title:"High-value opportunities",body:highOpp.length?`${highOpp.length} high-priority opportunity${highOpp.length===1?" is":"ies are"} ready for review.`:"No unworked high-priority opportunities.",view:"ai",color:highOpp.length?C.purple:C.green},
  ];
  return <div><Topbar title="KAI-TEQ Manager Agent" sub="A live management layer monitoring customers, work, finances, follow-ups and opportunities."/>
    <div className="grid grid-cols-4 gap-3 mb-5"><Stat label="Operating profit" value={gbp(profit)} accent={profit>=0?C.green:C.red}/><Stat label="Outstanding" value={gbp(outstanding)} accent={C.orange}/><Stat label="Meetings next 7 days" value={soon.length} accent={C.cyan}/><Stat label="Actions needing attention" value={overdue.length+sent.length+atRisk.length} accent={C.purple}/></div>
    <div className="grid grid-cols-2 gap-4 mb-5">{alerts.map(a=><Card key={a.title} className="cursor-pointer" style={{padding:16}}><div onClick={()=>setView(a.view)} className="flex items-start justify-between"><div><div style={{fontSize:13.5,fontWeight:650,color:C.text}}>{a.title}</div><div style={{fontSize:12,color:C.mut,lineHeight:1.55,marginTop:5}}>{a.body}</div></div><div style={{width:32,height:32,borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",background:`${a.color}18`,color:a.color,fontWeight:700}}>{a.n}</div></div></Card>)}</div>
    <Card style={{padding:18}}><div className="flex items-start gap-3"><Bot size={22} style={{color:C.cyan}}/><div><div style={{fontSize:13.5,fontWeight:650,color:C.text}}>Manager Agent architecture</div><div style={{fontSize:12,color:C.mut,lineHeight:1.65,marginTop:6}}>V9 establishes the monitoring and orchestration layer using live KAI-TEQ workspace data. This engine is deliberately deterministic for business-critical alerts. A later secure server-side AI connection can add natural-language briefings and allow this Manager Agent to coordinate specialist agents without exposing API secrets in the browser.</div></div></div></Card>
  </div>;
}

/* --------------------------- V9 Roles & Security --------------------------- */
function TeamSecurity({ role, email }) {
  const [members,setMembers]=useState([]);const [err,setErr]=useState("");
  useEffect(()=>{if(!supabase)return;supabase.from("workspace_members").select("user_id,email,display_name,role,active,created_at").order("created_at").then(({data,error})=>{if(error)setErr(error.message);else setMembers(data||[])})},[]);
  return <div><Topbar title="Team & Security" sub="Production access, roles and workspace security."/><div className="grid grid-cols-4 gap-3 mb-5"><Stat label="Your role" value={role.toUpperCase()} accent={C.cyan}/><Stat label="Active members" value={members.filter(m=>m.active).length} accent={C.green}/><Stat label="Authentication" value="Supabase" accent={C.purple}/><Stat label="Production domain" value={typeof window!=="undefined"&&window.location.hostname==="app.kai-teq.com"?"LIVE":"VERIFY"} accent={typeof window!=="undefined"&&window.location.hostname==="app.kai-teq.com"?C.green:C.amber}/></div>
    <Card style={{padding:0,overflow:"hidden",marginBottom:18}}>{members.map(m=><div key={m.user_id} className="grid items-center" style={{gridTemplateColumns:"1.3fr 1.7fr .7fr .6fr",padding:"13px 16px",borderBottom:`1px solid ${C.line}`,fontSize:12.5}}><span style={{color:C.text}}>{m.display_name||"KAI-TEQ member"}</span><span style={{color:C.mut}}>{m.email}</span><Chip label={m.role} color={m.role==="owner"?C.purple:m.role==="admin"?C.cyan:C.green}/><span style={{color:m.active?C.green:C.red}}>{m.active?"Active":"Disabled"}</span></div>)}{!members.length&&<div style={{padding:16,color:C.mut,fontSize:12}}>{err||"Run the V9 Supabase migration to activate role records."}</div>}</Card>
    <Card style={{padding:16}}><div style={{fontSize:13,fontWeight:650,color:C.text}}>Security model</div><div style={{fontSize:12,color:C.mut,lineHeight:1.65,marginTop:6}}>Owner and Admin can manage shared documents and administration. Members can use the operational workspace. Supabase Row Level Security restricts workspace state, document metadata and storage objects to active KAI-TEQ members. Microsoft client secrets and future AI provider secrets must never be placed in frontend environment variables.</div><div style={{fontSize:11,color:C.mut2,marginTop:8}}>Signed in: {email||"—"}</div></Card>
  </div>;
}
...
function WorkflowView() {
  return (<div><Topbar title="Workflow" sub="How KAI-TEQ runs — client delivery, and how we build the platform itself." />
    ...
    <Card style={{ padding: 16, marginTop: 16 }}>
      <div style={{ fontSize: 13, fontWeight: 600, color: C.text, marginBottom: 6 }}>V9 connected foundation</div>
      <p style={{ fontSize: 12.5, color: C.mut, lineHeight: 1.6 }}>The shared Supabase workspace, secure document storage, Outlook connector, role model and Manager Agent monitoring layer are now part of the operating system. Future AI specialist agents should sit behind a secure server-side service.</p></Card>
  </div>);
}
...
function EmailWorkspace() {
  const [account,setAccount]=useState(microsoftAccount());
  const saved=localStorage.getItem("kai_teq_ms_email_mailbox")||"";
  const [mailbox,setMailbox]=useState(saved);
  const [folder,setFolder]=useState("inbox"); const [messages,setMessages]=useState([]); const [selected,setSelected]=useState(null);
  const [busy,setBusy]=useState(false); const [msg,setMsg]=useState(""); const [compose,setCompose]=useState(false);
  const [draft,setDraft]=useState({to:"",cc:"",subject:"",body:""}); const [reply,setReply]=useState("");
  const [lastUpdated,setLastUpdated]=useState(null); const [search,setSearch]=useState(""); const [readFilter,setReadFilter]=useState("all");
  useEffect(()=>{restoreMicrosoftSession().then(setAccount).catch(()=>{})},[]);
  useEffect(()=>{localStorage.setItem("kai_teq_ms_email_mailbox",mailbox)},[mailbox]);
  const friendlyError=(e)=>{const t=e?.message||String(e);if(/403|ErrorAccessDenied|Access is denied/i.test(t)&&mailbox)return `Microsoft is connected, but this account does not currently have permission to access ${mailbox}. Check Full Access and Send As permissions in Microsoft 365.`;return t};
  const load=async(f=folder,{quiet=false}={})=>{if(!account)return;if(!quiet)setBusy(true);setMsg("");try{const rows=await listMailMessages({folder:f,mailbox});setMessages(rows);setLastUpdated(new Date());if(!quiet)setSelected(null)}catch(e){setMessages([]);setSelected(null);setMsg(friendlyError(e))}finally{if(!quiet)setBusy(false)}};
  useEffect(()=>{if(account)load(folder)},[account,folder,mailbox]);
  useEffect(()=>{if(!account)return;const id=setInterval(()=>load(folder,{quiet:true}),45000);return()=>clearInterval(id)},[account,folder,mailbox]);
  const open=async(m)=>{setSelected(m);if(!m.isRead&&folder==="inbox"){try{await markMailRead(m.id,{mailbox});setMessages(xs=>xs.map(x=>x.id===m.id?{...x,isRead:true}:x))}catch{}}};
  const connect=async()=>{setBusy(true);try{const a=await microsoftSignIn();setAccount(a);setMsg("Microsoft 365 connected.")}catch(e){setMsg(e.message||String(e))}finally{setBusy(false)}};
  const send=async()=>{setBusy(true);setMsg("");try{await sendMailMessage({...draft,mailbox});setCompose(false);setDraft({to:"",cc:"",subject:"",body:""});setMsg(`Email sent successfully${mailbox?` from ${mailbox}`:""}.`);if(folder==="sentitems")await load("sentitems")}catch(e){setMsg(friendlyError(e))}finally{setBusy(false)}};
  const sendReply=async()=>{if(!selected||!reply.trim())return;setBusy(true);try{await replyToMailMessage(selected.id,{body:reply,mailbox});setReply("");setMsg("Reply sent successfully.")}catch(e){setMsg(friendlyError(e))}finally{setBusy(false)}};
  const addr=(x)=>x?.emailAddress?.address||""; const name=(x)=>x?.emailAddress?.name||addr(x);
  const folderButtons=[["inbox","Inbox"],["sentitems","Sent"],["drafts","Drafts"],["deleteditems","Deleted"]];
  const shown=messages.filter(m=>{const q=search.trim().toLowerCase();const matches=!q||[m.subject,m.bodyPreview,name(m.from),(m.toRecipients||[]).map(addr).join(" ")].some(x=>String(x||"").toLowerCase().includes(q));const readOk=readFilter==="all"||(readFilter==="unread"?!m.isRead:m.isRead);return matches&&readOk});
  const mailboxLabel=mailbox||account?.username||account?.name||"Personal mailbox";
  if(!microsoftConfigured)return <div><Topbar title="Email" sub="Microsoft 365 email inside KAI-TEQ."/><Card style={{padding:18}}>Microsoft 365 setup is required.</Card></div>;
  return <div><Topbar title="Email" sub="Read, send and reply to Microsoft 365 email without leaving KAI-TEQ." action={<Btn primary onClick={()=>setCompose(true)}><Plus size={15}/> Compose</Btn>}/>
    <Card style={{padding:16,marginBottom:14,background:C.panel2}}><div className="flex items-start justify-between gap-4"><div style={{minWidth:0,flex:1}}><div className="flex items-center gap-2"><MicrosoftMark/><div style={{fontSize:13,fontWeight:650}}>Microsoft 365 / Outlook</div>{account&&<span style={{fontSize:10.5,fontWeight:700,color:C.green,border:`1px solid ${C.green}`,borderRadius:999,padding:"2px 8px"}}>Connected</span>}</div><div style={{fontSize:11.5,color:C.mut,marginTop:4,marginLeft:26}}>{account?`Connected as ${account.username||account.name}`:"Connect Microsoft 365 to use email."}</div></div>{account&&<div className="flex items-center gap-3"><div style={{textAlign:"right"}}><div className="flex items-center gap-2" style={{fontSize:11.5,fontWeight:650}}><span style={{width:9,height:9,borderRadius:"50%",background:C.green,boxShadow:`0 0 10px ${C.green}`}}/>Auto-updating</div><div style={{fontSize:10,color:C.mut2,marginTop:3}}>{lastUpdated?`Last updated: ${lastUpdated.toLocaleString("en-GB",{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"})}`:"Waiting for first sync…"}</div></div><Btn onClick={()=>load()}><RefreshCw size={14}/>{busy?"Refreshing…":"Refresh"}</Btn></div>}</div>
    {account&&<div className="mt-4" style={{maxWidth:650}}><Field label="Mailbox"><select style={inputStyle} value={mailbox} onChange={e=>setMailbox(e.target.value)}><option value="">{account.username||account.name||"My mailbox"} (Personal)</option><option value="hello@kai-teq.com">hello@kai-teq.com (Shared)</option></select></Field><div style={{fontSize:10.8,color:C.mut2,marginTop:6}}>Showing emails from {mailboxLabel}. The inbox updates automatically every 45 seconds.</div></div>}{!account&&<div className="mt-3"><Btn onClick={connect}><Cloud size={14}/>{busy?"Connecting…":"Connect Microsoft 365"}</Btn></div>}{msg&&<div style={{fontSize:11,color:/error|graph 4|graph 5|failed|permission|access denied/i.test(msg)?C.red:C.cyan,marginTop:8}}>{msg}</div>}</Card>
    <div className="flex justify-between gap-3 mb-3"><div className="flex gap-2">{folderButtons.map(([key,label])=><button key={key} onClick={()=>setFolder(key)} className="rounded-lg px-4 py-2" style={{background:folder===key?"rgba(20,160,255,.18)":C.panel2,border:`1px solid ${folder===key?C.cyan:C.line}`,fontSize:12,color:folder===key?C.text:C.mut}}>{label}</button>)}</div><div className="flex gap-2"><input style={{...inputStyle,width:280}} value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search emails…"/><select style={{...inputStyle,width:120}} value={readFilter} onChange={e=>setReadFilter(e.target.value)}><option value="all">All</option><option value="unread">Unread</option><option value="read">Read</option></select></div></div>
    <div className="grid gap-3" style={{gridTemplateColumns:selected?"minmax(320px,.8fr) minmax(420px,1.2fr)":"1fr"}}><Card style={{overflow:"hidden"}}>{shown.length===0?<div style={{padding:22,color:C.mut}}>{busy?"Loading messages…":"No messages in this view."}</div>:shown.map(m=>{const unread=folder==="inbox"&&!m.isRead;return <button key={m.id} onClick={()=>open(m)} className="w-full text-left" style={{padding:"13px 15px",borderBottom:`1px solid ${C.line}`,borderLeft:unread?`5px solid ${C.cyan}`:"5px solid transparent",background:selected?.id===m.id?"rgba(65,105,255,.14)":unread?"linear-gradient(90deg, rgba(0,169,255,.17), rgba(65,105,255,.07))":"transparent"}}><div className="flex items-center gap-3"><span style={{width:10,height:10,borderRadius:"50%",flex:"0 0 auto",background:unread?C.cyan:"rgba(113,135,178,.28)",boxShadow:unread?`0 0 9px ${C.cyan}`:"none"}}/><div style={{minWidth:0,flex:1}}><div className="flex justify-between gap-3"><div style={{fontSize:12.5,fontWeight:unread?750:500,color:C.text}}>{folder==="sentitems"?`To: ${(m.toRecipients||[]).map(addr).join(", ")||"—"}`:name(m.from)}</div><div style={{fontSize:10.5,color:unread?C.cyan:C.mut2,fontWeight:unread?700:400}}>{new Date(m.receivedDateTime||m.sentDateTime).toLocaleString("en-GB",{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"})}</div></div><div style={{fontSize:12,fontWeight:unread?750:500,marginTop:4}}>{m.subject||"(No subject)"}</div><div style={{fontSize:11,color:C.mut,marginTop:3,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{m.bodyPreview}</div></div></div></button>})}</Card>
    {selected&&<Card style={{padding:18}}><div className="flex justify-between gap-3"><div><h3 style={{fontSize:17,fontWeight:650}}>{selected.subject||"(No subject)"}</h3><div style={{fontSize:11.5,color:C.mut,marginTop:5}}>{folder==="sentitems"?`To: ${(selected.toRecipients||[]).map(addr).join(", ")}`:`From: ${name(selected.from)} <${addr(selected.from)}>`}</div></div><button onClick={()=>setSelected(null)} style={{color:C.mut,fontSize:12}}>Close</button></div><div style={{marginTop:18,fontSize:12.5,lineHeight:1.65,color:C.text}} dangerouslySetInnerHTML={{__html:selected.body?.contentType==="html"?selected.body.content:(selected.body?.content||selected.bodyPreview||"").replace(/\n/g,"<br/>")}}/>{folder==="inbox"&&<div style={{marginTop:20,borderTop:`1px solid ${C.line}`,paddingTop:14}}><textarea value={reply} onChange={e=>setReply(e.target.value)} placeholder="Write a reply…" style={{...inputStyle,minHeight:100,resize:"vertical"}}/><div className="flex justify-end mt-2"><Btn primary onClick={sendReply} disabled={busy}>Reply</Btn></div></div>}</Card>}</div>
    {compose&&<div className="fixed inset-0 flex items-center justify-center" style={{background:"rgba(2,6,18,.72)",zIndex:50}}><Card style={{width:"min(680px,92vw)",padding:20}}><div className="flex justify-between items-center mb-4"><div><h3 style={{fontSize:17,fontWeight:650}}>New email</h3><div style={{fontSize:10.8,color:C.mut,marginTop:3}}>Sending from {mailboxLabel}</div></div><button onClick={()=>setCompose(false)} style={{color:C.mut}}>Close</button></div><div className="grid gap-3"><Field label="To"><input style={inputStyle} value={draft.to} onChange={e=>setDraft({...draft,to:e.target.value})} placeholder="name@example.com"/></Field><Field label="Cc (optional)"><input style={inputStyle} value={draft.cc} onChange={e=>setDraft({...draft,cc:e.target.value})}/></Field><Field label="Subject"><input style={inputStyle} value={draft.subject} onChange={e=>setDraft({...draft,subject:e.target.value})}/></Field><Field label="Message"><textarea style={{...inputStyle,minHeight:180,resize:"vertical"}} value={draft.body} onChange={e=>setDraft({...draft,body:e.target.value})}/></Field></div><div className="flex justify-end gap-2 mt-4"><Btn onClick={()=>setCompose(false)}>Cancel</Btn><Btn primary onClick={send} disabled={busy}>{busy?"Sending…":"Send email"}</Btn></div></Card></div>}</div>;
}
...
export default function App() {
  ...
  const applySharedState = (next) => {
    setCustomers(next.customers); setProjects(next.projects); setProducts(next.products); setTasks(next.tasks);
    setMeetings(next.meetings); setProposals(next.proposals); setOpportunities(next.opportunities);
    setDiscovery(next.discovery); setExpenses(next.expenses);
  };
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      ...
      setUser(sessionUser ? userLabelFromEmail(sessionUser.email) : null);
      setAuthReady(true);
    });
    const { data: listener } = supabase.auth.onAuthStateChange((_event, session) => {
      ...
      setAuthUser(sessionUser);
      setUser(sessionUser ? userLabelFromEmail(sessionUser.email) : null);
      if (!sessionUser) setCloudReady(false);
      setAuthReady(true);
    });
  ...
  useEffect(() => {
    if (!authUser || !supabaseConfigured || !supabase) return;
    supabase.from("workspace_members").select("role,active").eq("user_id",authUser.id).maybeSingle().then(({data,error})=>{
      if (!error && data?.active) setRole(data.role || "member");
      else if (error) console.warn("V9 role lookup unavailable until migration is run", error.message);
    });
  }, [authUser?.id]);
  ...
  useEffect(() => {
    ...
    const localSnapshot = { customers, projects, products, tasks, meetings, proposals, opportunities, discovery, expenses };

    const hydrate = async () => {
      const { data: row, error } = await supabase.from("workspace_state").select("data, updated_at").eq("id", WORKSPACE_ID).maybeSingle();
      if (!active) return;
      if (error) {
        console.error("KAI-TEQ cloud load failed", error);
        setCloudReady(true);
        return;
      }
      if (row?.data) {
        const next = normaliseSharedData(row.data, localSnapshot);
        ...
        const seed = normaliseSharedData(localSnapshot, localSnapshot);
        const { error: insertError } = await supabase.from("workspace_state").upsert({ id: WORKSPACE_ID, data: seed, updated_by: authUser.id }, { onConflict: "id" });
        if (insertError) console.error("KAI-TEQ initial cloud save failed", insertError);
        else lastCloudJson.current = JSON.stringify(seed);
      }
      if (!active) return;
      setCloudReady(true);

      channel = supabase.channel("kai-teq-workspace")
        .on("postgres_changes", { event: "*", schema: "public", table: "workspace_state", filter: `id=eq.${WORKSPACE_ID}` }, (payload) => {
          if (!active || !payload?.new?.data) return;
          const next = normaliseSharedData(payload.new.data, localSnapshot);
          const json = JSON.stringify(next);
  ...
  useEffect(() => {
    const snapshot = { customers, projects, products, tasks, meetings, proposals, opportunities, discovery, expenses };
    const json = JSON.stringify(snapshot);
    try { localStorage.setItem(STORAGE_KEY, json); } catch (e) {}
    if (!authUser || !cloudReady || !supabaseConfigured || !supabase || applyingRemote.current || json === lastCloudJson.current) return;
    if (cloudSaveTimer.current) clearTimeout(cloudSaveTimer.current);
    cloudSaveTimer.current = setTimeout(async () => {
      const { error } = await supabase.from("workspace_state").upsert({ id: WORKSPACE_ID, data: snapshot, updated_by: authUser.id }, { onConflict: "id" });
      if (error) console.error("KAI-TEQ cloud save failed", error);
      else lastCloudJson.current = json;
    }, 450);
    return () => { if (cloudSaveTimer.current) clearTimeout(cloudSaveTimer.current); };
  }, [authUser?.id, cloudReady, customers, projects, products, tasks, meetings, proposals, opportunities, discovery, expenses]);

  /* projects */
  const setStage = (id, idx) => setProjects((ps) => ps.map((p) => p.id === id ? { ...p, stageIdx: Math.max(0, Math.min(STAGES.length - 1, idx)) } : p));
  const setProjectLifecycle = (id, lifecycle) => { const p=projects.find(x=>x.id===id); setProjects((ps) => ps.map((x) => x.id === id ? { ...x, lifecycle } : x)); if(p){ if(lifecycle==="maintenance") setCustomers(cs=>cs.map(c=>c.id===p.customerId?{...c,status:"Retainer"}:c)); if(lifecycle==="active") setCustomers(cs=>cs.map(c=>c.id===p.customerId?{...c,status:"Active"}:c)); } };
  const saveProject = (proj) => setProjects((ps) => ps.some((p) => p.id === proj.id) ? ps.map((p) => p.id === proj.id ? proj : p) : [proj, ...ps]);
  const deleteProject = (id) => { if(role==="member") return window.alert("Members cannot delete projects. Ask an Owner or Admin."); setProjects((ps) => ps.filter((p) => p.id !== id)); setTasks((ts) => ts.filter((t) => t.projectId !== id)); setMeetings((ms) => ms.filter((m) => m.projectId !== id)); setProposals((qs) => qs.filter((q) => q.projectId !== id)); setOpportunities((os) => os.filter((o) => o.projectId !== id)); setExpenses((es) => es.filter((e) => e.projectId !== id)); setDiscovery((d) => { const n = { ...d }; delete n[id]; return n; }); };
  ...
  const deleteCustomer = (id) => { if(role==="member") return window.alert("Members cannot delete customers. Ask an Owner or Admin."); const pids = projects.filter((p) => p.customerId === id).map((p) => p.id); setCustomers((cs) => cs.filter((c) => c.id !== id)); setProjects((ps) => ps.filter((p) => p.customerId !== id)); setTasks((ts) => ts.filter((t) => !pids.includes(t.projectId))); setMeetings((ms) => ms.filter((m) => m.customerId !== id)); setProposals((qs) => qs.filter((q) => q.customerId !== id)); setOpportunities((os) => os.filter((o) => o.customerId !== id)); setExpenses((es) => es.filter((e) => e.customerId !== id && !pids.includes(e.projectId))); setDiscovery((d) => { const n={...d}; pids.forEach((pid)=>delete n[pid]); return n; }); };
  /* tasks */
  const addTask = (task) => setTasks((ts) => [task, ...ts]);
  const moveTask = (id, col) => setTasks((ts) => ts.map((t) => t.id === id ? { ...t, col } : t));
  const deleteTask = (id) => { if(role==="member") return window.alert("Members cannot delete tasks. Ask an Owner or Admin."); setTasks((ts) => ts.filter((t) => t.id !== id)); };
  /* meetings */
  const addMeeting = (m) => setMeetings((ms) => [m, ...ms]);
  const importMeeting = (m) => setMeetings(ms=>[{id:"m"+Date.now()+Math.random().toString(36).slice(2,6),customerId:"",projectId:"",type:"Other",lead:user||"Tatenda",outcome:"scheduled",notes:{},...m},...ms]);
  const updateMeeting = (id, patch) => setMeetings((ms) => ms.map((m) => m.id === id ? { ...m, ...patch } : m));
  const deleteMeeting = (id) => { if(role==="member") return window.alert("Members cannot delete meetings. Ask an Owner or Admin."); setMeetings((ms) => ms.filter((m) => m.id !== id)); };
  const taskFromMeeting = (m) => { const proj = projects.find((p) => p.customerId === m.customerId); setModal({ type: "task", preset: proj ? proj.id : "", presetCustomerId: proj ? "" : m.customerId }); };
  /* proposals */
  const addProposal = (q) => setProposals((qs) => [q, ...qs]);
  const setProposalStatus = (id, status) => {
    const q=proposals.find(x=>x.id===id);
    setProposals((qs) => qs.map((x) => x.id === id ? { ...x, status } : x));
    if(!q) return;
    if(status==="sent") setCustomers(cs=>cs.map(c=>c.id===q.customerId?{...c,status:"Proposal"}:c));
    if(status==="accepted") {
      setCustomers(cs=>cs.map(c=>c.id===q.customerId?{...c,status:"Active",health:c.health==="risk"?"attention":c.health}:c));
      if(!projects.some(p=>p.proposalId===q.id)){ const c=customers.find(c=>c.id===q.customerId); setProjects(ps=>[{id:"p"+Date.now(),proposalId:q.id,customerId:q.customerId,name:q.title||`${c?.company||"Customer"} — Delivery`,lead:"Tatenda",health:"on",stageIdx:3,lifecycle:"active",tags:["Won proposal"],value:Number(q.value)||0,paid:0,retainer:0,revenueRecognised:0,revenueType:"Project delivery",invoiceStatus:"none"},...ps]); }
    }
  };
  const deleteProposal = (id) => { if(role==="member") return window.alert("Members cannot delete proposals. Ask an Owner or Admin."); setProposals((qs) => qs.filter((q) => q.id !== id)); };
  /* opportunities */
  const addOpp = (o) => setOpportunities((os) => [o, ...os]);
  const setOppStatus = (id, status) => setOpportunities((os) => os.map((o) => o.id === id ? { ...o, status } : o));
  const deleteOpp = (id) => { if(role==="member") return window.alert("Members cannot delete opportunities. Ask an Owner or Admin."); setOpportunities((os) => os.filter((o) => o.id !== id)); };
  /* discovery */
  ...
  const deleteExpense = (id) => { if(role==="member") return window.alert("Members cannot delete finance records. Ask an Owner or Admin."); setExpenses((es) => es.filter((e) => e.id !== id)); };
  /* products */
  ...
  const deleteProduct = (id) => { if(role==="member") return window.alert("Members cannot delete internal products. Ask an Owner or Admin."); setProducts((ps) => ps.filter((p) => p.id !== id)); setTasks((ts) => ts.filter((t) => t.projectId !== id)); };

  if (!authReady) return <AuthLoading />;
  if (!user) return <Login onLogin={setUser} />;
  if (supabaseConfigured && !cloudReady) return <AuthLoading />;

  const logout = async () => {
    if (supabase) await supabase.auth.signOut();
    setAuthUser(null); setUser(null); setCloudReady(false); setView("dashboard");
  };

  return (
    <div className="flex min-h-screen relative overflow-hidden" style={{ background: C.bg, fontFamily: FONT, color: C.text }}>
      <FuturisticBackdrop />
      <Sidebar view={view} setView={(v) => { setView(v); setActiveProject(null); setActiveProduct(null); if(v !== "customers") setActiveCustomer(null); }} user={user} role={role} onLogout={logout} />
      <main className="flex-1 overflow-auto relative" style={{ padding: "28px 32px", zIndex: 1 }}>
        {view === "pipeline" && <BusinessPipeline customers={customers} projects={projects} proposals={proposals} opportunities={opportunities} tasks={tasks} setView={setView} />}
        ...
        {view === "finance" && <Finance customers={customers} projects={projects} products={products} proposals={proposals} expenses={expenses} onInvoiceStatus={setInvoiceStatus} onAddExpense={addExpense} onDeleteExpense={deleteExpense} />}
        {view === "tasks" && <Tasks customers={customers} projects={projects} products={products} tasks={tasks} onMoveTask={moveTask} onDeleteTask={deleteTask} onNew={(pid) => setModal({ type: "task", preset: pid })} />}
        {view === "email" && <EmailWorkspace />}
        {view === "meetings" && <Meetings customers={customers} projects={projects} meetings={meetings} onUpdate={updateMeeting} onDelete={deleteMeeting} onNew={() => setModal({ type: "meeting" })} onCreateTask={taskFromMeeting} onImport={importMeeting} />}
        ...
        {view === "documents" && <DocumentsV9 customers={customers} projects={projects} products={products} expenses={expenses} role={role} />}
        {view === "manager" && <ManagerAgent customers={customers} projects={projects} tasks={tasks} meetings={meetings} proposals={proposals} opportunities={opportunities} expenses={expenses} setView={setView} />}
        {view === "admin" && <TeamSecurity role={role} email={authUser?.email} />}
      ...
      {modal?.type === "task" && <TaskModal customers={customers} projects={projects} products={products} preset={modal.preset} presetCustomerId={modal.presetCustomerId || ""} onClose={() => setModal(null)} onCreate={addTask} />}
      ...
      {modal?.type === "expense" && <ExpenseModal customers={customers} projects={projects} products={products} presetProjectId={modal.preset || ""} presetCustomerId={modal.presetCustomerId || ""} onClose={() => setModal(null)} onCreate={addExpense} />}
    </div>
  );
}

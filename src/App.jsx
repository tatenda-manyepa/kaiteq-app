...
import { microsoftConfigured, microsoftSignIn, microsoftSignOut, microsoftAccount, createOutlookEvent, importOutlookEvents } from "./microsoft";
import {
...
function OutlookConnectPanel({ meetings, onImport, onUpdate }) {
  const [account,setAccount]=useState(microsoftAccount()); const [busy,setBusy]=useState(false); const [msg,setMsg]=useState("");
  const connect=async()=>{setBusy(true);setMsg("");try{const a=await microsoftSignIn();setAccount(a);setMsg("Microsoft 365 connected.")}catch(e){setMsg(e.message||String(e))}finally{setBusy(false)}};
  const disconnect=async()=>{await microsoftSignOut();setAccount(null);setMsg("Disconnected from Microsoft 365.")};
  const imp=async()=>{setBusy(true);setMsg("");try{const rows=await importOutlookEvents();let n=0;for(const x of rows){if(!meetings.some(m=>m.outlookEventId===x.outlookEventId)){onImport(x);n++}}setMsg(`${n} Outlook event${n===1?"":"s"} imported.`)}catch(e){setMsg(e.message||String(e))}finally{setBusy(false)}};
  const push=async()=>{const pending=meetings.filter(m=>m.start&&!m.outlookEventId);if(!pending.length)return setMsg("No unsynced KAI-TEQ meetings to send.");setBusy(true);let n=0;try{for(const m of pending){const ev=await createOutlookEvent(m);onUpdate(m.id,{outlookEventId:ev.id,outlookWebLink:ev.webLink||""});n++}setMsg(`${n} meeting${n===1?"":"s"} sent to Outlook.`)}catch(e){setMsg(e.message||String(e))}finally{setBusy(false)}};
  if(!microsoftConfigured) return <Card style={{padding:14,marginBottom:16,background:C.panel2}}><div className="flex items-center justify-between"><div><div style={{fontSize:12.5,fontWeight:600,color:C.text}}>Microsoft 365 integration</div><div style={{fontSize:11.5,color:C.mut,marginTop:4}}>V9 integration is built. Add VITE_MS_CLIENT_ID and VITE_MS_TENANT_ID in Netlify, then register the redirect URI in Microsoft Entra.</div></div><Chip label="Setup required" color={C.amber}/></div></Card>;
  return <Card style={{padding:14,marginBottom:16,background:C.panel2}}><div className="flex items-center justify-between gap-4"><div><div style={{fontSize:12.5,fontWeight:600,color:C.text}}>Microsoft 365 / Outlook</div><div style={{fontSize:11.5,color:C.mut,marginTop:4}}>{account?`Connected as ${account.username||account.name||"Microsoft 365 user"}`:"Connect your KAI-TEQ Microsoft account to sync meetings."}</div>{msg&&<div style={{fontSize:11,color:msg.toLowerCase().includes("error")?C.red:C.cyan,marginTop:5}}>{msg}</div>}</div><div className="flex gap-2">{!account?<Btn onClick={connect}><Cloud size={14}/> {busy?"Connecting…":"Connect"}</Btn>:<><Btn onClick={imp}><RefreshCw size={14}/> Import Outlook</Btn><Btn onClick={push}><Upload size={14}/> Send to Outlook</Btn><button onClick={disconnect} style={{fontSize:11.5,color:C.mut}}>Disconnect</button></>}</div></div></Card>;
}

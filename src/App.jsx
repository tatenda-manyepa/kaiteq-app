...
const expenseTypeOf = (e) => e.costType || (e.productId ? "product" : e.projectId ? "project" : "overhead");
...
/* --------------------------- Finance --------------------------- */
function ExpenseModal({ projects, products = [], presetProjectId="", presetProductId="", onClose, onCreate }) {
  const presetProject=projects.find(p=>p.id===presetProjectId);
  const initialType=presetProjectId?"project":presetProductId?"product":"overhead";
  const [f,setF]=useState({label:"",amount:"",date:new Date().toISOString().slice(0,10),category:"Other",costType:initialType,projectId:presetProjectId,productId:presetProductId,customerId:presetProject?.customerId||""});
  const set=(k,v)=>setF(s=>({...s,[k]:v}));
  const save=()=>{if(!f.label.trim())return;const p=projects.find(x=>x.id===f.projectId);onCreate({id:"e"+Date.now(),label:f.label.trim(),amount:Number(f.amount)||0,date:f.date,category:f.category,costType:f.costType,projectId:f.costType==="project"?(f.projectId||""):"",productId:f.costType==="product"?(f.productId||""):"",customerId:f.costType==="project"?(p?.customerId||f.customerId||""):""});onClose();};
  return <Modal title="Add cost / expense" onClose={onClose} onSave={save} saveLabel="Add cost">
    <Field label="Description"><input style={inputStyle} value={f.label} onChange={e=>set("label",e.target.value)} placeholder="e.g. Microsoft 365 licences, domain renewal, contractor"/></Field>
    <div className="grid grid-cols-2 gap-3">
      ...
      <Field label="Cost type"><select style={inputStyle} value={f.costType} onChange={e=>{const t=e.target.value;setF(x=>({...x,costType:t,projectId:t==="project"?x.projectId:"",productId:t==="product"?x.productId:"",customerId:t==="project"?x.customerId:""}))}}>{EXPENSE_TYPES.map(x=><option key={x.key} value={x.key}>{x.label}</option>)}</select></Field>
    ...
    <div className="rounded-lg p-3 mt-2" style={{background:C.panel2,border:`1px solid ${C.line}`,fontSize:11.5,color:C.mut,lineHeight:1.5}}>{(EXPENSE_TYPES.find(x=>x.key===f.costType)||EXPENSE_TYPES[0]).desc}. Business overheads include costs such as domains, Microsoft 365, accounting, insurance and general software.</div>
  </Modal>;
}
function Finance({ customers, projects, products = [], proposals, expenses, onInvoiceStatus, onAddExpense, onDeleteExpense }) {
  ...
  const exp=projectExp+overheadExp+productExp;
  ...
  const costList=(type,title,subtitle)=>{const rows=expenses.filter(e=>expenseTypeOf(e)===type);const total=rows.reduce((a,e)=>a+(Number(e.amount)||0),0);return <div><div className="flex items-end justify-between mb-2"><div><h3 style={{fontSize:13,fontWeight:600,color:C.text}}>{title}</h3><div style={{fontSize:10.5,color:C.mut2,marginTop:2}}>{subtitle}</div></div><span style={{fontSize:13,fontWeight:600,color:C.amber}}>{gbp(total)}</span></div><Card style={{padding:0,overflow:"hidden"}}>{rows.map(e=>{const link=type==="project"?projName(projects,e.projectId):type==="product"?(products.find(p=>p.id===e.productId)?.name||"Internal product"):"KAI-TEQ";return <div key={e.id} className="flex items-center justify-between" style={{padding:"11px 14px",borderBottom:`1px solid ${C.line}`}}><div><div style={{fontSize:12.5,color:C.text}}>{e.label}</div><div style={{fontSize:10.5,color:C.mut2}}>{e.category||"Other"} · {link} · {e.date}</div></div><div className="flex items-center gap-3"><span style={{fontSize:12.5,color:C.amber}}>{gbp(e.amount)}</span><button onClick={()=>onDeleteExpense(e.id)} style={{color:C.mut2}}><X size={14}/></button></div></div>})}{!rows.length&&<div style={{padding:14,fontSize:12,color:C.mut2}}>No costs recorded.</div>}</Card></div>};
  return <div><Topbar title="Finance" sub="KAI-TEQ company performance — revenue, project costs, overheads, internal products and operating profit." action={<Btn onClick={()=>setShowExp(true)}><Plus size={15}/> Add cost</Btn>}/>
    ...
    <div className="grid grid-cols-3 gap-4 mb-7">{costList("overhead","Business overheads","Company-wide running costs")}{costList("project","Project costs","Direct client delivery costs")}{costList("product","Internal product costs","KAI-TEQ-owned product investment")}</div>
    ...
    {showExp&&<ExpenseModal projects={projects} products={products} onClose={()=>setShowExp(false)} onCreate={onAddExpense}/>}</div>;
}
...
export default function App() {
  ...
  return (
    <div className="flex min-h-screen relative overflow-hidden" style={{ background: C.bg, fontFamily: FONT, color: C.text }}>
      ...
      {modal?.type === "expense" && <ExpenseModal projects={projects} products={products} presetProjectId={modal.preset || ""} onClose={() => setModal(null)} onCreate={addExpense} />}
    </div>
  );
}

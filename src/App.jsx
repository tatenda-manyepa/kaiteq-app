...
function MicrosoftMark() {
  return <span aria-hidden="true" style={{display:"grid",gridTemplateColumns:"repeat(2,7px)",gridTemplateRows:"repeat(2,7px)",gap:2,width:16,height:16}}>
...
function EmailWorkspace() {
  const [account,setAccount]=useState(microsoftAccount());
  const [mailbox,setMailbox]=useState(()=>localStorage.getItem("kai_teq_ms_email_mailbox")||"");
  ...
  const [search,setSearch]=useState(""); const [readFilter,setReadFilter]=useState("all"); const [lastUpdated,setLastUpdated]=useState(null);
  ...
  const load=async(f=folder,quiet=false)=>{if(!account)return;if(!quiet)setBusy(true);setMsg("");try{setMessages([]);const rows=await listMailMessages({folder:f,mailbox});setMessages(rows||[]);setSelected(null);setLastUpdated(new Date())}catch(e){setMessages([]);const text=e?.message||String(e);setMsg(mailbox&&/403|accessdenied|erroraccessdenied/i.test(text)?`Microsoft 365 is connected, but this account does not currently have permission to access ${mailbox}. Check Full Access / Send As permissions for the shared mailbox.`:text)}finally{if(!quiet)setBusy(false)}};
  useEffect(()=>{if(account)load(folder)},[account,folder,mailbox]);
  useEffect(()=>{if(!account)return;const timer=setInterval(()=>load(folder,true),45000);return()=>clearInterval(timer)},[account,folder,mailbox]);
  const open=async(m)=>{setSelected(m);if(!m.isRead&&folder==="inbox"){try{await markMailRead(m.id,{mailbox});setMessages(xs=>xs.map(x=>x.id===m.id?{...x,isRead:true}:x));setSelected({...m,isRead:true})}catch{}}};
  ...
  const send=async()=>{setBusy(true);setMsg("");try{await sendMailMessage({...draft,mailbox});setCompose(false);setDraft({to:"",cc:"",subject:"",body:""});setMsg("Email sent successfully.");if(folder==="sentitems")await load("sentitems")}catch(e){setMsg(e.message||String(e))}finally{setBusy(false)}};
  const sendReply=async()=>{if(!selected||!reply.trim())return;setBusy(true);try{await replyToMailMessage(selected.id,{body:reply,mailbox});setReply("");setMsg("Reply sent successfully.")}catch(e){setMsg(e.message||String(e))}finally{setBusy(false)}};
  const addr=(x)=>x?.emailAddress?.address||""; const name=(x)=>x?.emailAddress?.name||addr(x);
  const visible=messages.filter(m=>{const q=search.trim().toLowerCase();const matches=!q||[m.subject,m.bodyPreview,name(m.from),(m.toRecipients||[]).map(addr).join(" ")].some(v=>String(v||"").toLowerCase().includes(q));const readOk=readFilter==="all"||(readFilter==="unread"?!m.isRead:m.isRead);return matches&&readOk});
  const folders=[{key:"inbox",label:"Inbox"},{key:"sentitems",label:"Sent"},{key:"drafts",label:"Drafts"},{key:"deleteditems",label:"Deleted"}];
  ...
  return <div><Topbar title="Email" sub="Read, send and reply to Microsoft 365 email without leaving KAI-TEQ." action={<Btn primary onClick={()=>setCompose(true)}><Plus size={15}/> Compose</Btn>}/>
    <Card style={{padding:16,marginBottom:14,background:C.panel2}}><div className="flex items-start justify-between gap-4 flex-wrap"><div style={{minWidth:280,flex:1}}><div className="flex items-center gap-2"><MicrosoftMark/><div style={{fontSize:12.5,fontWeight:650}}>Microsoft 365 / Outlook</div>{account&&<span style={{fontSize:10.5,fontWeight:700,color:C.green,border:`1px solid ${C.green}`,borderRadius:999,padding:"2px 8px"}}>Connected</span>}</div><div style={{fontSize:11.5,color:C.mut,marginTop:4,marginLeft:24}}>{account?`Connected as ${account.username||account.name}`:"Connect Microsoft 365 to use email."}</div></div><div className="flex items-center gap-4">{account&&<div><div className="flex items-center gap-2" style={{fontSize:11.5,fontWeight:600}}><span style={{width:9,height:9,borderRadius:"50%",background:C.green,display:"inline-block"}}/>Auto-updating</div><div style={{fontSize:10.5,color:C.mut2,marginTop:3}}>{lastUpdated?`Last updated: ${lastUpdated.toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit"})}`:"Updates every 45 seconds"}</div></div>}{!account?<Btn onClick={connect}><Cloud size={14}/>{busy?"Connecting…":"Connect"}</Btn>:<Btn onClick={()=>load()}><RefreshCw size={14}/>{busy?"Refreshing…":"Refresh"}</Btn>}</div></div>
    {account&&<div className="mt-3" style={{maxWidth:650}}><Field label="Mailbox"><select style={inputStyle} value={mailbox} onChange={e=>setMailbox(e.target.value)}><option value="">My personal mailbox</option><option value="hello@kai-teq.com">hello@kai-teq.com (Shared)</option></select></Field><div style={{fontSize:10.8,color:C.mut2,marginTop:5}}>Showing emails from {mailbox||"your personal mailbox"}.</div></div>}{msg&&<div style={{fontSize:11,color:/error|graph 4|graph 5|failed|permission/i.test(msg)?C.red:C.cyan,marginTop:8}}>{msg}</div>}</Card>
    <div className="flex items-center justify-between gap-3 mb-3 flex-wrap"><div className="flex gap-2 flex-wrap">{folders.map(x=><button key={x.key} onClick={()=>setFolder(x.key)} className="rounded-lg px-4 py-2" style={{background:folder===x.key?"rgba(65,105,255,.16)":C.panel2,border:`1px solid ${folder===x.key?C.blue:C.line}`,fontSize:12,color:folder===x.key?C.text:C.mut}}>{x.label}</button>)}</div><div className="flex gap-2"><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search emails…" style={{...inputStyle,width:250}}/><select value={readFilter} onChange={e=>setReadFilter(e.target.value)} style={{...inputStyle,width:120}}><option value="all">All</option><option value="unread">Unread</option><option value="read">Read</option></select></div></div>
    <div className="grid gap-3" style={{gridTemplateColumns:selected?"minmax(320px,.8fr) minmax(420px,1.2fr)":"1fr"}}><Card style={{overflow:"hidden"}}>{visible.length===0?<div style={{padding:22,color:C.mut}}>{busy?"Loading messages…":"No messages in this view."}</div>:visible.map(m=>{const unread=folder==="inbox"&&!m.isRead;return <button key={m.id} onClick={()=>open(m)} className="w-full text-left" style={{padding:"13px 15px",borderBottom:`1px solid ${C.line}`,borderLeft:unread?`4px solid ${C.cyan}`:"4px solid transparent",background:selected?.id===m.id?"rgba(65,105,255,.14)":unread?"linear-gradient(90deg, rgba(0,184,255,.14), rgba(65,105,255,.06))":"transparent"}}><div className="flex justify-between gap-3"><div className="flex items-center gap-2">{folder==="inbox"&&<span style={{width:9,height:9,borderRadius:"50%",background:unread?C.cyan:C.line,display:"inline-block",flex:"0 0 auto"}}/>}<div style={{fontSize:12.5,fontWeight:unread?750:500,color:C.text}}>{folder==="sentitems"?`To: ${(m.toRecipients||[]).map(addr).join(", ")||"—"}`:name(m.from)}</div></div><div style={{fontSize:10.5,fontWeight:unread?700:400,color:unread?C.cyan:C.mut2}}>{new Date(m.receivedDateTime||m.sentDateTime).toLocaleString("en-GB",{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"})}</div></div><div style={{fontSize:12,fontWeight:unread?750:500,marginTop:4,marginLeft:folder==="inbox"?17:0}}>{m.subject||"(No subject)"}</div><div style={{fontSize:11,color:C.mut,marginTop:3,marginLeft:folder==="inbox"?17:0,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{m.bodyPreview}</div></button>})}</Card>
    {selected&&<Card style={{padding:18}}><div className="flex justify-between gap-3"><div><h3 style={{fontSize:17,fontWeight:650}}>{selected.subject||"(No subject)"}</h3><div style={{fontSize:11.5,color:C.mut,marginTop:5}}>{folder==="sentitems"?`To: ${(selected.toRecipients||[]).map(addr).join(", ")}`:`From: ${name(selected.from)} <${addr(selected.from)}>`}</div></div><button onClick={()=>setSelected(null)} style={{color:C.mut,fontSize:12}}>Close</button></div><div style={{marginTop:18,fontSize:12.5,lineHeight:1.65,color:C.text}} dangerouslySetInnerHTML={{__html:selected.body?.contentType?.toLowerCase()==="html"?selected.body.content:(selected.body?.content||selected.bodyPreview||"").replace(/\n/g,"<br/>")}}/>{folder==="inbox"&&<div style={{marginTop:20,borderTop:`1px solid ${C.line}`,paddingTop:14}}><textarea value={reply} onChange={e=>setReply(e.target.value)} placeholder="Write a reply…" style={{...inputStyle,minHeight:100,resize:"vertical"}}/><div className="flex justify-end mt-2"><Btn primary onClick={sendReply} disabled={busy}>Reply</Btn></div></div>}</Card>}</div>
    {compose&&<div className="fixed inset-0 flex items-center justify-center" style={{background:"rgba(2,6,18,.72)",zIndex:50}}><Card style={{width:"min(680px,92vw)",padding:20}}><div className="flex justify-between items-center mb-4"><h3 style={{fontSize:17,fontWeight:650}}>New email {mailbox?`from ${mailbox}`:""}</h3><button onClick={()=>setCompose(false)} style={{color:C.mut}}>Close</button></div><div className="grid gap-3"><Field label="To"><input style={inputStyle} value={draft.to} onChange={e=>setDraft({...draft,to:e.target.value})} placeholder="name@example.com"/></Field><Field label="Cc (optional)"><input style={inputStyle} value={draft.cc} onChange={e=>setDraft({...draft,cc:e.target.value})}/></Field><Field label="Subject"><input style={inputStyle} value={draft.subject} onChange={e=>setDraft({...draft,subject:e.target.value})}/></Field><Field label="Message"><textarea style={{...inputStyle,minHeight:180,resize:"vertical"}} value={draft.body} onChange={e=>setDraft({...draft,body:e.target.value})}/></Field></div><div className="flex justify-end gap-2 mt-4"><Btn onClick={()=>setCompose(false)}>Cancel</Btn><Btn primary onClick={send} disabled={busy}>{busy?"Sending…":"Send email"}</Btn></div></Card></div>}</div>;
}

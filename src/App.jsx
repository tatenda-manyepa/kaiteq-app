...
/* --------------------------- Internal Products --------------------------- */
const PRODUCT_TYPES = ["Website", "Web App", "Mobile App", "SaaS", "Software", "AI Product", "Other"];
const BUSINESS_MODELS = ["Internal only", "Free", "One-off", "Subscription", "Usage-based", "Freemium", "Other"];
const PRODUCT_TABS = ["Overview", "Development", "Roadmap", "Commercial", "Releases", "Documents"];
function productDefaults(base = {}) { return { id: "prod" + Date.now(), name: "", type: "Web App", customType: "", lead: "Tatenda", stageIdx: 0, tags: [], oneOff: 0, monthly: 0, customersActive: 0, status: "idea", description: "", notes: "", businessModel: "Internal only", repoUrl: "", liveUrl: "", hosting: "", techStack: "", version: "", startDate: "", targetLaunch: "", actualLaunch: "", roadmap: "", releases: "", documents: "", costsMonthly: 0, ...base }; }
function ProductModal({ product, onClose, onSave }) {
  const [f, setF] = useState(productDefaults(product || {}));
  const set = (k, v) => setF((x) => ({ ...x, [k]: v }));
  const save = () => { if (!f.name.trim()) return; onSave({ ...f, name: f.name.trim(), oneOff: Number(f.oneOff) || 0, monthly: Number(f.monthly) || 0, costsMonthly: Number(f.costsMonthly) || 0, customersActive: Number(f.customersActive) || 0, stageIdx: Number(f.stageIdx), tags: Array.isArray(f.tags) ? f.tags : String(f.tags).split(",").map((t) => t.trim()).filter(Boolean) }); onClose(); };
  const tagsStr = Array.isArray(f.tags) ? f.tags.join(", ") : f.tags;
  return (<Modal wide title={product ? "Edit product" : "New internal product"} onClose={onClose} onSave={save} saveLabel={product ? "Save product" : "Create product"}>
    <div style={{ fontSize: 12, color: C.cyan, fontWeight: 600, marginBottom: 4 }}>Product essentials</div>
    <div className="grid grid-cols-2 gap-3">
      <Field label="Product name"><input style={inputStyle} value={f.name} onChange={(e) => set("name", e.target.value)} placeholder="e.g. KAI-TEQ Admin Suite" /></Field>
      <Field label="Product type"><select style={inputStyle} value={f.type} onChange={(e) => set("type", e.target.value)}>{PRODUCT_TYPES.map((x) => <option key={x}>{x}</option>)}</select>{f.type === "Other" && <input style={{ ...inputStyle, marginTop: 8 }} value={f.customType || ""} onChange={(e) => set("customType", e.target.value)} placeholder="Specify product type" />}</Field>
      <Field label="Lead / owner"><select style={inputStyle} value={f.lead} onChange={(e) => set("lead", e.target.value)}>{TEAM.map((t) => <option key={t}>{t}</option>)}</select></Field>
      <Field label="Status"><select style={inputStyle} value={f.status} onChange={(e) => set("status", e.target.value)}>{PRODUCT_STATUS.map((x) => <option key={x.key} value={x.key}>{x.label}</option>)}</select></Field>
      <Field label="Development stage"><select style={inputStyle} value={f.stageIdx} onChange={(e) => set("stageIdx", e.target.value)}>{STAGES.map((x, i) => <option key={x.key} value={i}>{i + 1}. {x.name}</option>)}</select></Field>
      <Field label="Business model"><select style={inputStyle} value={f.businessModel} onChange={(e) => set("businessModel", e.target.value)}>{BUSINESS_MODELS.map((x) => <option key={x}>{x}</option>)}</select></Field>
    </div>
    <Field label="Short description"><textarea rows={2} style={{ ...inputStyle, resize: "vertical" }} value={f.description} onChange={(e) => set("description", e.target.value)} placeholder="What is the product and who is it for?" /></Field>
    {product && <>
      <div style={{ fontSize: 12, color: C.cyan, fontWeight: 600, marginTop: 6 }}>Development</div>
      <div className="grid grid-cols-2 gap-3">
        <Field label="Repository / GitHub URL"><input style={inputStyle} value={f.repoUrl} onChange={(e) => set("repoUrl", e.target.value)} placeholder="https://github.com/..." /></Field>
        <Field label="Live / production URL"><input style={inputStyle} value={f.liveUrl} onChange={(e) => set("liveUrl", e.target.value)} placeholder="https://..." /></Field>
        <Field label="Hosting / platform"><input style={inputStyle} value={f.hosting} onChange={(e) => set("hosting", e.target.value)} placeholder="Netlify, Cloudflare, Azure..." /></Field>
        <Field label="Tech stack"><input style={inputStyle} value={f.techStack} onChange={(e) => set("techStack", e.target.value)} placeholder="React, Supabase, Node..." /></Field>
        <Field label="Current version"><input style={inputStyle} value={f.version} onChange={(e) => set("version", e.target.value)} placeholder="v1.0" /></Field>
        <Field label="Tags"><input style={inputStyle} value={tagsStr} onChange={(e) => set("tags", e.target.value)} placeholder="SaaS, AI, Automation" /></Field>
        <Field label="Development start"><input style={inputStyle} type="date" value={f.startDate} onChange={(e) => set("startDate", e.target.value)} /></Field>
        <Field label="Target launch"><input style={inputStyle} type="date" value={f.targetLaunch} onChange={(e) => set("targetLaunch", e.target.value)} /></Field>
      </div>
      <div style={{ fontSize: 12, color: C.cyan, fontWeight: 600, marginTop: 6 }}>Commercial</div>
      <div className="grid grid-cols-4 gap-3">
        ...
        <Field label="Monthly product costs (£)"><input style={inputStyle} type="number" value={f.costsMonthly} onChange={(e) => set("costsMonthly", e.target.value)} /></Field>
      </div>
      <Field label="Roadmap / planned features"><textarea rows={3} style={{ ...inputStyle, resize: "vertical" }} value={f.roadmap} onChange={(e) => set("roadmap", e.target.value)} placeholder="One feature or milestone per line" /></Field>
      <Field label="Release history"><textarea rows={3} style={{ ...inputStyle, resize: "vertical" }} value={f.releases} onChange={(e) => set("releases", e.target.value)} placeholder="v1.0 — Initial launch..." /></Field>
      <Field label="Documents / links"><textarea rows={2} style={{ ...inputStyle, resize: "vertical" }} value={f.documents} onChange={(e) => set("documents", e.target.value)} placeholder="Specs, research, design docs, links..." /></Field>
      <Field label="Notes"><textarea rows={3} style={{ ...inputStyle, resize: "vertical" }} value={f.notes} onChange={(e) => set("notes", e.target.value)} /></Field>
    </>}
  </Modal>);
}
function ProductDetail({ product, tasks, onBack, onSetStage, onMoveTask, onDelete, onEdit, onAddTask }) {
  const p = productDefaults(product), [tab, setTab] = useState("Overview");
  const displayType = p.type === "Other" && p.customType?.trim() ? p.customType.trim() : p.type;
  const ptasks = tasks.filter((t) => t.projectId === p.id), st = PRODUCT_STATUS.find((x) => x.key === p.status) || PRODUCT_STATUS[0];
  const mrr = (p.monthly || 0) * (p.customersActive || 0), monthlyProfit = mrr - (p.costsMonthly || 0), arr = mrr * 12;
  const TextBlock = ({ text, empty }) => <div style={{ whiteSpace: "pre-wrap", fontSize: 13, color: text ? C.text : C.mut2, lineHeight: 1.7 }}>{text || empty}</div>;
  const LinkRow = ({ label, value }) => <div className="flex items-center justify-between py-3" style={{ borderBottom: `1px solid ${C.line}` }}><span style={{ fontSize: 12, color: C.mut }}>{label}</span>{value ? <a href={value} target="_blank" rel="noreferrer" style={{ color: C.cyan, fontSize: 12, maxWidth: "70%", overflow: "hidden", textOverflow: "ellipsis" }}>{value}</a> : <span style={{ color: C.mut2, fontSize: 12 }}>Not added</span>}</div>;
  return (<div>
    ...
    <div className="flex items-start justify-between mb-5"><div><div className="flex items-center gap-3"><h1 style={{ fontSize: 22, fontWeight: 600, color: C.text }}>{p.name}</h1><Chip label={st.label} color={st.color} /><Chip label={displayType} color={C.cyan} /></div><div className="flex items-center gap-2 mt-1" style={{ fontSize: 13, color: C.mut }}><Package size={14} /> KAI-TEQ owned product · <Avatar name={p.lead} size={18} /> {p.lead}</div></div>
      <div className="flex items-center gap-2"><button onClick={() => onEdit(p)} className="flex items-center gap-2 rounded-lg px-3 py-2" style={{ background: C.panel2, color: C.text, border: `1px solid ${C.line}`, fontSize: 12 }}><Pencil size={14} /> Manage product</button><button onClick={() => { if (window.confirm("Delete this product and its linked tasks?")) { onDelete(p.id); onBack(); } }} className="flex items-center justify-center rounded-lg" style={{ width: 38, height: 38, background: C.panel2, color: C.mut, border: `1px solid ${C.line}` }}><Trash2 size={15} /></button></div></div>
    {p.description && <p style={{ color: C.mut, fontSize: 13, lineHeight: 1.6, margin: "-10px 0 18px", maxWidth: 850 }}>{p.description}</p>}
    <div className="flex gap-1 mb-5" style={{ borderBottom: `1px solid ${C.line}` }}>{PRODUCT_TABS.map((x) => <button key={x} onClick={() => setTab(x)} style={{ padding: "10px 14px", fontSize: 12.5, color: tab === x ? C.cyan : C.mut, borderBottom: tab === x ? `2px solid ${C.cyan}` : "2px solid transparent", fontWeight: tab === x ? 600 : 400 }}>{x}</button>)}</div>
    {tab === "Overview" && <><div className="grid grid-cols-4 gap-4 mb-5"><Stat label="Status" value={st.label} accent={st.color} /><Stat label="Business model" value={p.businessModel} accent={C.cyan} /><Stat label="Paying customers" value={p.customersActive || 0} accent={C.amber} /><Stat label="MRR" value={gbp(mrr)} accent={C.green} /></div>
      <Card style={{ padding: 20, marginBottom: 20 }}><div className="flex items-center justify-between mb-4"><h2 style={{ fontSize: 14, fontWeight: 600 }}>Development pipeline</h2><span style={{ fontSize: 12, color: C.mut }}>{progressOf(p)}% complete</span></div><div className="grid grid-cols-8 gap-1">{STAGES.map((x, i) => { const done=i<p.stageIdx, active=i===p.stageIdx; return <div key={x.key} className="text-center cursor-pointer" onClick={() => onSetStage(p.id,i)}><div className="mx-auto flex items-center justify-center rounded-full mb-2" style={{width:32,height:32,background:done||active?GRAD:C.panel2,border:done||active?"none":`1px solid ${C.line}`}}>{done?<CircleCheck size={16} color="#fff"/>:active?<CircleDot size={16} color="#fff"/>:<Circle size={14} color={C.mut2}/>}</div><div style={{fontSize:9.5,color:done||active?C.text:C.mut2}}>{x.name}</div></div>})}</div></Card>
      <div className="flex items-center justify-between mb-3"><h2 style={{ fontSize: 14, fontWeight: 600 }}>Build board</h2><button onClick={() => onAddTask(p.id)} className="flex items-center gap-1.5 rounded-lg px-3 py-1.5" style={{ background: C.panel2, color: C.text, fontSize: 12, border: `1px solid ${C.line}` }}><Plus size={13} /> Add task</button></div><div className="grid grid-cols-5 gap-2.5">{COLS.map((col) => { const items=ptasks.filter((t)=>t.col===col.key); return <div key={col.key} className="rounded-xl p-2.5" style={{background:C.panel,border:`1px solid ${C.line}`}}><div className="flex items-center justify-between mb-3"><span style={{fontSize:11,fontWeight:600,color:col.color}}>{col.label}</span><span style={{fontSize:11,color:C.mut2}}>{items.length}</span></div><div className="flex flex-col gap-2">{items.map((t)=><TaskMini key={t.id} t={t} onMove={onMoveTask}/>)}{!items.length&&<div style={{fontSize:11,color:C.mut2}}>—</div>}</div></div>})}</div></>}
    {tab === "Development" && <div className="grid grid-cols-2 gap-4"><Card style={{padding:18}}><h2 style={{fontSize:14,fontWeight:600,marginBottom:8}}>Technical details</h2><LinkRow label="Repository" value={p.repoUrl}/><LinkRow label="Live product" value={p.liveUrl}/><div className="py-3"><div style={{fontSize:12,color:C.mut}}>Hosting / platform</div><div style={{fontSize:13,marginTop:5}}>{p.hosting||"Not added"}</div></div><div className="py-3"><div style={{fontSize:12,color:C.mut}}>Tech stack</div><div style={{fontSize:13,marginTop:5}}>{p.techStack||"Not added"}</div></div></Card><Card style={{padding:18}}><h2 style={{fontSize:14,fontWeight:600,marginBottom:12}}>Delivery</h2><div style={{fontSize:12,color:C.mut}}>Current version</div><div style={{fontSize:20,fontWeight:600,margin:"5px 0 18px"}}>{p.version||"—"}</div><div className="grid grid-cols-2 gap-4"><div><div style={{fontSize:12,color:C.mut}}>Started</div><div style={{fontSize:13,marginTop:5}}>{p.startDate||"—"}</div></div><div><div style={{fontSize:12,color:C.mut}}>Target launch</div><div style={{fontSize:13,marginTop:5}}>{p.targetLaunch||"—"}</div></div></div></Card></div>}
    {tab === "Roadmap" && <Card style={{padding:20}}><h2 style={{fontSize:14,fontWeight:600,marginBottom:12}}>Roadmap & planned features</h2><TextBlock text={p.roadmap} empty="No roadmap items yet. Use Manage product to add planned features and milestones." /></Card>}
    {tab === "Commercial" && <><div className="grid grid-cols-4 gap-4 mb-5"><Stat label="MRR" value={gbp(mrr)} accent={C.green}/><Stat label="ARR" value={gbp(arr)} accent={C.cyan}/><Stat label="Monthly costs" value={gbp(p.costsMonthly||0)} accent={C.amber}/><Stat label="Monthly profit" value={gbp(monthlyProfit)} accent={monthlyProfit>=0?C.green:C.red}/></div><Card style={{padding:18}}><div className="grid grid-cols-4 gap-4"><div><div style={{fontSize:12,color:C.mut}}>Business model</div><div style={{fontSize:14,marginTop:5}}>{p.businessModel}</div></div><div><div style={{fontSize:12,color:C.mut}}>One-off price</div><div style={{fontSize:14,marginTop:5}}>{gbp(p.oneOff)}</div></div><div><div style={{fontSize:12,color:C.mut}}>Monthly price</div><div style={{fontSize:14,marginTop:5}}>{gbp(p.monthly)}</div></div><div><div style={{fontSize:12,color:C.mut}}>Paying customers</div><div style={{fontSize:14,marginTop:5}}>{p.customersActive||0}</div></div></div></Card></>}
    {tab === "Releases" && <Card style={{padding:20}}><h2 style={{fontSize:14,fontWeight:600,marginBottom:12}}>Release history</h2><TextBlock text={p.releases} empty="No releases recorded yet." /></Card>}
    {tab === "Documents" && <Card style={{padding:20}}><h2 style={{fontSize:14,fontWeight:600,marginBottom:12}}>Product documents & links</h2><TextBlock text={p.documents} empty="No product documents or links added yet." />{p.notes&&<><div style={{height:1,background:C.line,margin:"20px 0"}}/><h3 style={{fontSize:12,color:C.mut,marginBottom:8}}>Notes</h3><TextBlock text={p.notes}/></>}</Card>}
  </div>);
}
function Products({ products, tasks, activeProduct, setActiveProduct, onSetStage, onMoveTask, onDelete, onNew, onEdit, onAddTask }) {
  const p = products.find((x) => x.id === activeProduct); if (p) return <ProductDetail product={p} tasks={tasks} onBack={() => setActiveProduct(null)} onSetStage={onSetStage} onMoveTask={onMoveTask} onDelete={onDelete} onEdit={onEdit} onAddTask={onAddTask} />;
  const mrrTotal=products.reduce((a,p)=>a+(Number(p.monthly)||0)*(Number(p.customersActive)||0),0);
  return (<div><Topbar title="Internal Products" sub="KAI-TEQ owned websites, apps, SaaS and software — built once, improved and monetised as products." action={<Btn primary onClick={onNew}><Plus size={15}/> New product</Btn>}/><div className="grid grid-cols-3 gap-4 mb-5"><Stat label="Products" value={products.length} accent={C.cyan}/><Stat label="Live" value={products.filter((x)=>x.status==="live").length} accent={C.green}/><Stat label="Total MRR" value={gbp(mrrTotal)} accent={C.purple}/></div><div className="flex flex-col gap-3">{products.map((raw)=>{const pr=productDefaults(raw),st=PRODUCT_STATUS.find((x)=>x.key===pr.status)||PRODUCT_STATUS[0],displayType=pr.type === "Other" && pr.customType?.trim() ? pr.customType.trim() : pr.type;return <Card key={pr.id} className="cursor-pointer" style={{padding:18}}><div onClick={()=>setActiveProduct(pr.id)}><div className="flex items-center justify-between mb-2"><div className="flex items-center gap-3"><span style={{fontSize:15,fontWeight:600}}>{pr.name}</span><Chip label={st.label} color={st.color}/><Chip label={displayType} color={C.cyan}/></div><div className="flex items-center gap-2"><Avatar name={pr.lead} size={20}/><span style={{fontSize:12,color:C.mut}}>{pr.businessModel}</span></div></div>{pr.description&&<div style={{fontSize:12.5,color:C.mut,marginBottom:10}}>{pr.description}</div>}<div className="flex items-center justify-between mb-3"><span style={{fontSize:12,color:C.cyan}}>{STAGES[pr.stageIdx].name}</span><span style={{fontSize:12,color:C.mut2}}>{pr.customersActive||0} paying · {gbp((pr.monthly||0)*(pr.customersActive||0))} MRR</span></div><Pipeline stageIdx={pr.stageIdx}/></div></Card>})}{!products.length&&<Card style={{padding:20}}><span style={{color:C.mut,fontSize:13}}>No products yet — create the first KAI-TEQ owned product.</span></Card>}</div></div>);
}
...
export default function App() {
  ...
  return (
    <div className="flex min-h-screen relative overflow-hidden" style={{ background: C.bg, fontFamily: FONT, color: C.text }}>
      <FuturisticBackdrop />
      <Sidebar view={view} setView={(v) => { setView(v); setActiveProject(null); setActiveProduct(null); if(v !== "customers") setActiveCustomer(null); }} user={user} onLogout={() => { setUser(null); setView("dashboard"); }} />

...
React + Vite. Authentication is connected to Supabase. Business data is still saved in browser localStorage until the shared database migration is completed.
...
The current version uses Supabase authentication but still stores business data in browser `localStorage`. Cascading deletion is therefore enforced in application state, not by a database. Before using the platform as the shared live business system, move the business data into Supabase and enforce customer/project relationships with foreign keys and database-level cascade rules.
